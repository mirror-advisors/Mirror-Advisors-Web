...
// Call the SalesIQ JS API to identify the visitor. The script is already
// loaded site-wide via components/Layout.js, so $zoho.salesiq is normally
// available; ready() queues the calls if not.
function _chatIdentifyVisitor(payload) {
  ...
  window.$zoho.salesiq.ready(function () {
    try {
      ...
      if (!v) return;
      if (payload.name  && typeof v.name  === 'function') v.name(String(payload.name));
      if (payload.email && typeof v.email === 'function') v.email(String(payload.email));
      // VERIFY: SalesIQ phone method name (Zoho docs:
      // https://www.zoho.com/salesiq/help/developer-section/js-api.html ) —
      // it's most likely visitor.contactnumber() but confirm in the live
      // SalesIQ build. If their build uses visitor.phone(...) instead, change
      // this one line.
      if (payload.phone && typeof v.contactnumber === 'function') v.contactnumber(String(payload.phone));
    } catch (e) { console.warn('[chat] SalesIQ visitor.* error:', e); }
  });
}
