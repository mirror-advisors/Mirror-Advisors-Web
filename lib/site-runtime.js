...
function _chatIdentifyVisitor(payload) {
  ...
  if (typeof window.$zoho.salesiq.ready !== 'function') {
    console.warn('[lead-capture] $zoho.salesiq.ready is NOT a function — identify aborted.');
    return;
  }
  console.log('[lead-capture] $zoho.salesiq.ready is a function — registering callback now.');
  window.$zoho.salesiq.ready(function () {
    console.log('[lead-capture] ready() CALLBACK FIRING NOW (SalesIQ confirms initialized).');
    try {
      var v = window.$zoho.salesiq.visitor;
      if (!v) {
        console.warn('[lead-capture] $zoho.salesiq.visitor is UNAVAILABLE inside ready() — identify aborted.');
        return;
      }
      ...
      if (typeof v.info === 'function') {
        ...
        try {
          var r = v.info(bulk);
          console.log('[lead-capture] ✓ visitor.info(', bulk, ') called. Return value:', r);
        } catch (e) {
          console.warn('[lead-capture] ✗ visitor.info() THREW:', e);
        }
      } else {
        console.warn('[lead-capture] visitor.info is NOT a function on this SalesIQ build — skipping bulk call.');
      }

      // FALLBACK / SUPPLEMENT: individual setters. Same values, idempotent.
      if (name  && typeof v.name      === 'function') { try { var rn = v.name(name);       console.log('[lead-capture] ✓ visitor.name(\''+name+'\') →', rn); } catch (e) { console.warn('[lead-capture] ✗ visitor.name THREW:', e); } }
      if (fn    && typeof v.firstname === 'function') { try { var rf = v.firstname(fn);    console.log('[lead-capture] ✓ visitor.firstname(\''+fn+'\') →', rf); } catch (e) { console.warn('[lead-capture] ✗ visitor.firstname THREW:', e); } }
      if (ln    && typeof v.lastname  === 'function') { try { var rl = v.lastname(ln);    console.log('[lead-capture] ✓ visitor.lastname(\''+ln+'\') →', rl); } catch (e) { console.warn('[lead-capture] ✗ visitor.lastname THREW:', e); } }
      if (email && typeof v.email     === 'function') { try { var re_ = v.email(email);    console.log('[lead-capture] ✓ visitor.email(\''+email+'\') →', re_); } catch (e) { console.warn('[lead-capture] ✗ visitor.email THREW:', e); } }
      if (phone && typeof v.contactnumber === 'function') { try { var rp = v.contactnumber(phone); console.log('[lead-capture] ✓ visitor.contactnumber(\''+phone+'\') →', rp); } catch (e) { console.warn('[lead-capture] ✗ visitor.contactnumber THREW:', e); } }
      ...
      try {
        ...
        console.log('[lead-capture] identify COMPLETE for siqid:', siqid);
      } catch (_) {}
    } catch (e) { console.warn('[lead-capture] identify outer error:', e); }
  });
  console.log('[lead-capture] ready() callback registered. Will fire as soon as SalesIQ confirms it is initialized.');
}
