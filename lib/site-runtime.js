...
window.submitForm = function(e){
  ...
  // Collect selected services from the multi-select dropdown (checkboxes
  // inside #svcChips). Falls back to the legacy active-chip selector for
  // any cached HTML still using the old button row.
...
// ── CHATBOT ENGINE — AI mode (Gemini via /api/chat) ──────────────────────────
// Replaces the previous decision-tree engine. The widget now:
//   1. Opens with a friendly greeting + a row of "quick starter" buttons
//      seeded from window._CHAT_TREE.topics (just labels — clicks send the
//      label as a question to the AI, instead of looking up a canned answer).
//   2. Lets the visitor type any question into the input row. Each question
//      is POSTed to /api/chat with the recent conversation history. The
//      serverless function calls Gemini grounded in the site's own copy and
//      returns the answer.
//   3. Shows a typing indicator while the answer streams back.
//   4. Keeps a rolling _chatHistory the API uses for follow-up context.
//
// Why this design:
//   - Existing widget styling, drag-snap launcher, and open/close behaviour
//     are preserved — only the message logic changes.
//   - The starter buttons keep the discoverability of the old menu without
//     forcing visitors through a tree.
//   - All AI calls go through OUR serverless function so the Gemini key
//     never reaches the browser.
var _chatOpen     = false;
var _chatHistory  = [];   // [{ role: 'user'|'model', text: string }, ...]
var _chatBusy     = false;
var _chatInputBound = false;
var _CHAT_API_ENDPOINT = '/api/chat';
var _CHAT_HISTORY_TURNS_TO_SEND = 12; // keep matched with API's MAX_HISTORY_TURNS
...
window.chatToggle = function() {
  ...
  if (_chatOpen) {
    _initChatInput();
    if (_chatHistory.length === 0) _chatStart();
...
// POST to /api/chat. Sends recent history so Gemini handles follow-ups.
...
// Light formatting for bot answers: escape first, then convert paragraph
// breaks to <br><br>, bullet lines (- or •) to a simple <ul>, and turn bare
// URLs into clickable links. NO arbitrary HTML — keeps the bot from injecting
// styled content into the page.
function _chatFormatBot(text) {
  ...
  html = html.replace(/\n{2,}/g, '<br><br>').replace(/(?<!>)\n(?!<)/g, '<br>');
  // Bare URLs → links (after escaping, & is already &amp; so the regex is safe)
  html = html.replace(/(https?:\/\/[^\s<]+)/g, function (u) {
    return '<a href="' + u + '" target="_blank" rel="noopener noreferrer" style="color:var(--t)">' + u + '</a>';
  });
  return html;
}
