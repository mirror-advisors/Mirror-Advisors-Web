...
window.adminEditCase = function(idx){
  console.log('[admin] adminEditCase (simple) idx:', idx);
  var cases = window._CASES || [];
  if(idx < 0 || idx >= cases.length){
    console.warn('[admin] No case at idx', idx);
    return;
  }
  var c = cases[idx];
  _adminIdx = idx;
  ...
  if(!eb){ console.warn('[admin] No #adminEditorBody'); return; }
  ...
  console.log('[admin] Editor populated. Body length:', eb.innerHTML.length);
};
