...
function _renderSocialIcons() {
  var keys = ['linkedin','facebook','instagram','x','youtube','google','tiktok'];
...
window.slSaveAll = function() {
  ['linkedin','facebook','instagram','x','youtube','google','tiktok'].forEach(function(k) {
...
// ── BACKGROUNDS ADMIN ────────────────────────────────────────────────────────
var _bgSelectedPage = 'home';
var _bgPendingFile  = null;

var _BG_PAGE_LABELS = {
  home:'Home', services:'Services', capabilities:'Capabilities',
  cases:'Case Studies', stack:'Our Stack', contact:'Contact',
  erp:'ERP Projects', infinity:'Infinity Mirror', bankhours:'Bank of Hours',
  support:'Support Only', zoho_crm:'Zoho CRM', zoho_books:'Zoho Books',
  zoho_analytics:'Analytics', zoho_flow:'Zoho Flow', zoho_desk:'Zoho Desk',
  zoho_sign:'Zoho Sign', zoho_projects:'Zoho Projects',
  zoho_inventory:'Inventory', zoho_people:'Zoho People',
  zoho_recruit:'Zoho Recruit', zoho_expense:'Zoho Expense',
  zoho_creator:'Zoho Creator'
};

window.adminRenderBgDash = function(container){
  container.innerHTML = '';
  ...
  topbar.style.cssText = 'flex-shrink:0;padding:14px 20px 10px;border-bottom:1px solid rgba(255,255,255,.08);display:flex;align-items:center;gap:10px;';
  topbar.innerHTML =
    '<div style="font-family:Montserrat,sans-serif;font-size:13px;font-weight:800;color:var(--tx);">Page Backgrounds</div>'
    + '<div style="font-size:12px;color:var(--dim);margin-left:4px;">Click any page to edit its background</div>';

  // Grid
  var grid = document.createElement('div');
  grid.style.cssText = 'flex:1;overflow-y:auto;padding:20px;';
  grid.innerHTML = _bgRenderGrid();

  container.appendChild(topbar);
  container.appendChild(grid);
};

function _bgRenderGrid(){
  var bgs  = window._PAGE_BACKGROUNDS || {};
  var keys = Object.keys(_BG_PAGE_LABELS);
  return '<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(160px,1fr));gap:12px;">'
    + keys.map(function(k){
        var bg  = bgs[k] || {};
        var has = bg.type && bg.type !== 'none' && bg.src;
        var thumb = has
          ? (bg.type === 'video'
              ? '<video autoplay loop muted playsinline src="'+bg.src+'" style="width:100%;height:100%;object-fit:cover;display:block;"></video>'
              : '<img src="'+bg.src+'" style="width:100%;height:100%;object-fit:cover;display:block;">')
          : '<div style="display:flex;align-items:center;justify-content:center;height:100%;color:var(--dim);font-size:11px;font-family:Montserrat,sans-serif;">No BG</div>';
        return '<div onclick="bgOpenEditor(\''+k+'\')" style="border-radius:10px;overflow:hidden;border:1px solid rgba(255,255,255,'+(has?'.18':'.08')+');background:rgba(255,255,255,.03);cursor:pointer;transition:border-color .2s,transform .15s;" onmouseover="this.style.borderColor=\'rgba(236,169,52,.4)\';this.style.transform=\'translateY(-2px)\'" onmouseout="this.style.borderColor=\'rgba(255,255,255,'+(has?'.18':'.08')+')\';this.style.transform=\'\'">'
          + '<div style="height:90px;overflow:hidden;position:relative;">'
          + thumb
          + (has ? '<div style="position:absolute;top:6px;right:6px;width:8px;height:8px;border-radius:50%;background:#4CAF50;box-shadow:0 0 6px rgba(76,175,80,.8);"></div>' : '')
          + '</div>'
          + '<div style="padding:8px 10px;font-family:Montserrat,sans-serif;font-size:10px;font-weight:800;color:var(--tx);display:flex;align-items:center;justify-content:space-between;">'
          + '<span>'+_BG_PAGE_LABELS[k]+'</span>'
          + '<span style="font-size:9px;color:'+(has?'#4CAF50':'var(--dim)')+'">'+( has ? '● SET' : '○ none' )+'</span>'
          + '</div>'
          + '</div>';
      }).join('')
    + '</div>';
...
window.bgOpenEditor = function(k){
  var container = document.getElementById('adminBgContent');
  if(!container) return;
  container.innerHTML = '';
  container.style.display = 'flex';
  container.style.flexDirection = 'column';
  ...
  var bgs = window._PAGE_BACKGROUNDS || {};
  var bg  = bgs[k] || {};
  var activeType    = bg.type    || 'none';
  var activeOpacity = bg.opacity !== undefined ? bg.opacity : 0.15;
  var activeSrc     = bg.src     || null;

  // Header
  var header = document.createElement('div');
  header.style.cssText = 'flex-shrink:0;padding:14px 20px;border-bottom:1px solid rgba(255,255,255,.08);display:flex;align-items:center;gap:12px;';
  header.innerHTML =
    '<button onclick="window.adminRenderBgDash(document.getElementById(\'adminBgContent\'))" style="background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.1);color:var(--dim);border-radius:7px;padding:5px 12px;cursor:pointer;font-size:12px;font-family:Montserrat,sans-serif;font-weight:700;">← Back</button>'
    + '<div style="font-family:Montserrat,sans-serif;font-size:14px;font-weight:800;color:var(--tx);">'+_BG_PAGE_LABELS[k]+'</div>'
    + (activeSrc ? '<span style="font-size:9px;padding:3px 9px;border-radius:100px;background:rgba(76,175,80,.12);border:1px solid rgba(76,175,80,.25);color:#4CAF50;font-family:Montserrat,sans-serif;font-weight:700;">● BG SET</span>' : '');
  ...
  edPanel.style.cssText = 'flex:1;overflow-y:auto;padding:24px;max-width:480px;';
  edPanel.id = 'bgEditorPanel_'+k;

  function buildEditorHTML(type, src, opacity){
    var op = opacity !== undefined ? opacity : 0.15;
    return '<div class="admin-field"><label style="font-family:Montserrat,sans-serif;font-size:10px;font-weight:800;text-transform:uppercase;letter-spacing:.09em;color:var(--dim);">Background Type</label>'
      + '<div style="display:flex;gap:6px;margin-top:6px;">'
      + ['image','video','none'].map(function(t){
          var labels = {image:'🖼 Image / GIF', video:'🎬 Video', none:'✕ None'};
          return '<button onclick="bgSetTypeEditor(\''+k+'\',\''+t+'\')" style="flex:1;padding:8px;border-radius:8px;border:1px solid rgba(255,255,255,'+(type===t?'.3':'.1')+');background:rgba(255,255,255,'+(type===t?'.1':'.03')+');color:'+(type===t?'var(--tx)':'var(--dim)')+';font-family:Montserrat,sans-serif;font-size:11px;font-weight:700;cursor:pointer;">'+labels[t]+'</button>';
        }).join('')
      + '</div></div>'
      + (type !== 'none' ?
        '<div class="admin-field">'
        + '<label style="font-family:Montserrat,sans-serif;font-size:10px;font-weight:800;text-transform:uppercase;letter-spacing:.09em;color:var(--dim);">Upload '+(type==='video'?'Video (MP4, WebM)':'Image or GIF')+'</label>'
        + '<div onclick="document.getElementById(\'bgFileInput_'+k+'\').click()" style="margin-top:6px;border:2px dashed rgba(255,255,255,.12);border-radius:10px;padding:24px;text-align:center;cursor:pointer;transition:all .2s;" onmouseover="this.style.borderColor=\'rgba(236,169,52,.4)\'" onmouseout="this.style.borderColor=\'rgba(255,255,255,.12)\'">'
        + '<input type="file" id="bgFileInput_'+k+'" style="display:none;" accept="'+(type==='video'?'video/*':'image/*,.gif')+'" onchange="bgHandleFileEditor(\''+k+'\',this)">'
        + '<div style="font-size:24px;margin-bottom:6px;">'+(src?'✓':'📁')+'</div>'
        + '<div style="font-size:12px;color:var(--dim);">'+(src?'File uploaded — click to replace':'Click to upload')+'</div>'
        + '</div></div>'
        : '')
      + (src ?
        '<div class="admin-field"><label style="font-family:Montserrat,sans-serif;font-size:10px;font-weight:800;text-transform:uppercase;letter-spacing:.09em;color:var(--dim);">Page Overlay Opacity <span style="color:var(--tx);" id="bgOpVal_'+k+'">'+Math.round(op*100)+'%</span></label>'
        + '<input type="range" min="0" max="0.95" step="0.01" value="'+op+'" oninput="bgUpdateOpacityEditor(\''+k+'\',this.value)" style="width:100%;margin-top:8px;accent-color:var(--t);">'
        + '<div style="font-size:11px;color:var(--dim);margin-top:4px;">Lower = background more visible. Applies dark overlay on top of background.</div>'
        + '</div>'
        : '')
      + (src ?
        '<div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:8px;">'
        + '<button class="admin-save" onclick="bgApplyEditor(\''+k+'\')">✓ Apply to Page</button>'
        + '<button onclick="bgPreviewEditor(\''+k+'\')" style="background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.12);color:var(--mid);border-radius:8px;padding:10px 18px;font-family:Montserrat,sans-serif;font-weight:700;font-size:12px;cursor:pointer;">👁 Preview on Site</button>'
        + '<button class="admin-del-btn" onclick="bgClearEditor(\''+k+'\')">Remove BG</button>'
        + '</div>'
        : (type !== 'none' ? '<p style="font-size:12px;color:var(--dim);margin-top:8px;">Upload a file to apply it.</p>' : '<p style="font-size:12px;color:var(--dim);margin-top:8px;">No background will be shown for this page.</p>'))
      ;
  }

  edPanel.innerHTML = buildEditorHTML(activeType, activeSrc, activeOpacity);
  edPanel._buildEditorHTML = buildEditorHTML;

  // Preview panel
  var pvPanel = document.createElement('div');
  pvPanel.style.cssText = 'flex:1;display:flex;flex-direction:column;border-left:1px solid rgba(255,255,255,.08);overflow:hidden;min-width:240px;';
  pvPanel.innerHTML =
    '<div style="flex-shrink:0;padding:12px 16px;font-family:Montserrat,sans-serif;font-size:11px;font-weight:800;text-transform:uppercase;letter-spacing:.08em;color:var(--dim);border-bottom:1px solid rgba(255,255,255,.08);">Preview</div>'
    + '<div id="bgPreviewPanel_'+k+'" style="flex:1;overflow:hidden;position:relative;background:#080B1A;">'
    + _bgBuildPreview(activeSrc, activeType, activeOpacity)
    + '</div>';

  body.appendChild(edPanel);
  body.appendChild(pvPanel);
  container.appendChild(header);
  container.appendChild(body);

  // Store state
  window._bgEditorState = window._bgEditorState || {};
  window._bgEditorState[k] = { type: activeType, src: activeSrc, opacity: activeOpacity };
};

function _bgBuildPreview(src, type, opacity){
  if(!src || type === 'none') return '<div style="display:flex;align-items:center;justify-content:center;height:100%;color:var(--dim);font-size:12px;">No background set</div>';
  var media = type === 'video'
    ? '<video autoplay loop muted playsinline src="'+src+'" style="position:absolute;inset:0;width:100%;height:100%;object-fit:cover;"></video>'
    : '<img src="'+src+'" style="position:absolute;inset:0;width:100%;height:100%;object-fit:cover;">';
  return media + '<div style="position:absolute;inset:0;background:rgba(8,11,26,'+((opacity||0.15).toFixed(2))+');"></div>'
    + '<div style="position:absolute;bottom:12px;left:50%;transform:translateX(-50%);font-size:10px;color:rgba(255,255,255,.5);font-family:Montserrat,sans-serif;white-space:nowrap;">Overlay: '+Math.round((opacity||0.15)*100)+'%</div>';
}

window.bgSetTypeEditor = function(k, type){
  if(!window._bgEditorState) window._bgEditorState = {};
  if(!window._bgEditorState[k]) window._bgEditorState[k] = {};
  window._bgEditorState[k].type = type;
  if(type === 'none'){
    window._bgEditorState[k].src = null;
  }
  var panel = document.getElementById('bgEditorPanel_'+k);
  if(panel){
    var st = window._bgEditorState[k];
    panel.innerHTML = panel._buildEditorHTML(st.type, st.src, st.opacity);
    panel._buildEditorHTML = panel._buildEditorHTML; // preserve reference
  }
};

window.bgHandleFileEditor = function(k, input){
  ...
  reader.onload = function(e){
    ...
    if(!window._bgEditorState) window._bgEditorState = {};
    if(!window._bgEditorState[k]) window._bgEditorState[k] = {};
    window._bgEditorState[k].src  = e.target.result;
    window._bgEditorState[k].type = type;
    // Rebuild editor
    var panel = document.getElementById('bgEditorPanel_'+k);
    if(panel && panel._buildEditorHTML){
      var st = window._bgEditorState[k];
      panel.innerHTML = panel._buildEditorHTML(st.type, st.src, st.opacity||0.15);
    }
    // Update preview
    var pvPanel = document.getElementById('bgPreviewPanel_'+k);
    if(pvPanel){
      var st = window._bgEditorState[k];
      pvPanel.innerHTML = _bgBuildPreview(st.src, st.type, st.opacity||0.15);
    }
  };
  reader.readAsDataURL(file);
};

window.bgUpdateOpacityEditor = function(k, val){
  var opacity = parseFloat(val);
  if(!window._bgEditorState) window._bgEditorState = {};
  if(!window._bgEditorState[k]) window._bgEditorState[k] = {};
  window._bgEditorState[k].opacity = opacity;
  var valEl = document.getElementById('bgOpVal_'+k);
  if(valEl) valEl.textContent = Math.round(opacity*100)+'%';
  var pvPanel = document.getElementById('bgPreviewPanel_'+k);
  if(pvPanel){
    var st = window._bgEditorState[k];
    pvPanel.innerHTML = _bgBuildPreview(st.src, st.type, opacity);
  }
};

window.bgApplyEditor = function(k){
  var st = window._bgEditorState && window._bgEditorState[k];
  if(!st || !st.src) return;
  if(!window._PAGE_BACKGROUNDS) window._PAGE_BACKGROUNDS = {};
  window._PAGE_BACKGROUNDS[k] = { type: st.type, src: st.src, opacity: st.opacity||0.15 };
  _applyPageBackground(window._currentPage || 'home');
  // Persist to Supabase so every visitor sees the update.
  _persist('page_backgrounds', window._PAGE_BACKGROUNDS);
  _adminToast('Background applied to '+_BG_PAGE_LABELS[k]+' \u2713');
};

window.bgPreviewEditor = function(k){
  // Close admin and navigate to that page
  closeAdmin();
  go(k);
};

window.bgClearEditor = function(k){
  if(!confirm('Remove background from '+_BG_PAGE_LABELS[k]+'?')) return;
  if(window._PAGE_BACKGROUNDS) window._PAGE_BACKGROUNDS[k] = { type:'none', src:null, opacity:0.15 };
  if(window._bgEditorState && window._bgEditorState[k]){
    window._bgEditorState[k] = { type:'none', src:null, opacity:0.15 };
  }
  _applyPageBackground(window._currentPage || 'home');
  // Persist the cleared background.
  _persist('page_backgrounds', window._PAGE_BACKGROUNDS);
  _adminToast('Background removed \u2713');
  // Rebuild editor
  var container = document.getElementById('adminBgContent');
  if(container) window.adminRenderBgDash(container);
};
...
window.submitForm = function(e){
  ...
  var timeline = document.getElementById('timeline') ? document.getElementById('timeline').value.trim() : '';
  // Collect selected service chips
  var chips    = document.querySelectorAll('#svcChips .svc-chip.active');
  var services = Array.from(chips).map(function(c){ return c.textContent.trim(); });
  ...
  // Persist to Supabase so admins see it from any device.
  // RLS allows anonymous INSERT into the submissions table.
  try {
    insertSubmission(submission).then(function (ok) {
      if (ok) console.log('[submissions] ✓ Saved to Supabase:', submission.id);
      else    console.warn('[submissions] Could not persist submission; admin will only see it locally.');
    });
  } catch (e) {
    console.warn('[submissions] persist threw:', e);
  }

  var btn = document.getElementById('submitBtn');
  if(btn){ btn.textContent='Sending...'; btn.style.opacity='.7'; btn.disabled=true; }
  setTimeout(function(){
...
function _buildCasePage(idx){
  ...
  html += '<div class="cd-page">'
    ...
    + '<button class="cd-back" onclick="go(\'cases\')">'
  ...
  html += '<div class="cd-metrics-strip">'
    + '<div class="cd-metric-card"><div class="cd-metric-val" style="color:'+tc+'">'+c.m1+'</div><div class="cd-metric-lbl">'+c.ml1+'</div></div>'
    + '<div class="cd-metric-card"><div class="cd-metric-val" style="color:'+tc+'">'+c.m2+'</div><div class="cd-metric-lbl">'+c.ml2+'</div></div>'
    + '<div class="cd-metric-card"><div class="cd-metric-val" style="color:#4CAF50">100%</div><div class="cd-metric-lbl">On-Time Delivery</div></div>'
    + '<div class="cd-metric-card"><div class="cd-metric-val" style="color:#6B9FD4">0</div><div class="cd-metric-lbl">Scope Creep</div></div>'
    + '</div>';
  ...
  blocks.forEach(function(b){
    if(b.type === 'metrics') return; // shown in strip above
  ...
  // ── Footer ────────────────────────────────────────────────────────────────
  html += '<div class="pg-footer"><div class="pg-foot-in">'
    + '<div class="pg-foot-top">'
    + '<div class="pft-brand">'
    + '<div class="logo" onclick="go(\'home\')" style="display:inline-flex;align-items:center;gap:10px;cursor:pointer;margin-bottom:16px">'
    + '<img src="'+_LOGO_SRC+'" alt="Mirror Advisors" class="logo-img">'
    + '<span class="logo-n" style="font-size:16px">Mirror Advisors</span></div>'
    + '<p class="pft-brand-desc">We build the technology infrastructure that lets ambitious businesses move faster — custom AI apps, enterprise ERP, and deep systems integration.</p>'
    + '<div class="pft-tagline">Zoho One &bull; Anthropic Claude AI</div>'
    + '<div class="pft-social"></div>'
    + '</div>'
    + '<div><div class="pft-col-title">Services</div><div class="pft-links">'
    + '<a onclick="go(\'services\')">Scope Engagement</a>'
    + '<a onclick="go(\'erp\')">ERP Projects</a>'
    + '<a onclick="go(\'infinity\')">Infinity Mirror</a>'
    + '<a onclick="go(\'bankhours\')">Bank of Hours</a>'
    + '<a onclick="go(\'support\')">Support Only</a>'
    + '</div></div>'
    + '<div><div class="pft-col-title">Company</div><div class="pft-links">'
    + '<a onclick="go(\'capabilities\')">Capabilities</a>'
    + '<a onclick="go(\'cases\')">Case Studies</a>'
    + '<a onclick="go(\'stack\')">Technology</a>'
    + '<a onclick="go(\'contact\')">Contact Us</a>'
    + '</div></div>'
    + '</div>'
    + '<div class="pg-foot-bottom">'
    + '<div class="foot-copy">&#169; 2025 Mirror Advisors. All rights reserved.</div>'
    + '<div class="foot-badges"><span class="foot-badge">Zoho Partner</span><span class="foot-badge">Claude AI</span></div>'
    + '</div></div></div>'
...
window.chatToggle = function() {
  ...
  if (_chatOpen && _chatHistory.length === 0) _chatStart();
};

function _chatStart() {
  var tree = window._CHAT_TREE;
  if (!tree) return;
  _chatAddBot(tree.opening, tree.topics.map(function(t) {
    return { label: t.label, id: t.id, type: 'topic' };
  }));
}

function _chatAddBot(text, choices) {
  ...
  setTimeout(function() {
    ...
    if (choices && choices.length) {
      ...
      cl.className = 'chat-choices';
      choices.forEach(function(c, i) {
        var btn = document.createElement('button');
        btn.className = 'chat-choice' + (c.cta ? ' cta-choice' : c.secondary ? ' secondary' : '');
        btn.textContent = c.label;
        btn.style.animationDelay = (i * 0.06) + 's';
        btn.addEventListener('click', function() { _chatChoose(c); });
        cl.appendChild(btn);
      });
      msgs.appendChild(cl);
    }
...
function _chatChoose(choice) {
  // Disable all current buttons
  var btns = document.querySelectorAll('#chatMessages .chat-choice');
  btns.forEach(function(b) { b.disabled = true; b.style.opacity = '.4'; b.style.cursor = 'default'; });

  // Editor-driven routing (route flag wins over legacy fields)
  if (choice.route === 'cta')   { go('contact'); window.chatToggle(); return; }
  if (choice.route === 'goto' && choice.goto)  { go(choice.goto); window.chatToggle(); return; }
  if (choice.route === 'reset') {
    var treeR = window._CHAT_TREE;
    _chatAddBot('What else would you like to know?', treeR.topics.map(function(t) {
      return { label: t.label, id: t.id, type: 'topic' };
    }));
    return;
  }
  ...
  if (choice.cta) { go('contact'); window.chatToggle(); return; }
  if (choice.goto) { go(choice.goto); window.chatToggle(); return; }
  if (choice.type === 'reset') {
    var tree = window._CHAT_TREE;
    _chatAddBot('What else would you like to know?', tree.topics.map(function(t) {
      return { label: t.label, id: t.id, type: 'topic' };
    }));
    return;
  }
  ...
  if (choice.type === 'topic') {
    var topic = null;
    for (var i = 0; i < tree.topics.length; i++) {
      if (tree.topics[i].id === choice.id) { topic = tree.topics[i]; break; }
    }
    if (!topic) return;
    _chatAddBot(topic.answer, _chatBuildChoices(topic.choices));
    return;
  }

  // Node selected
  var nodeId = choice.nodeId || choice.id;
  var node   = tree.nodes[nodeId];
  if (!node) {
    _chatAddBot(tree.fallback, [{ label: '📞 Book a Strategy Session', cta: true }]);
    return;
  }
  // route 'answer-end' = show answer with no follow-up buttons
  var extra;
  if (choice.route === 'answer-end') {
    extra = null;
  } else {
    extra = (node.choices && node.choices.length) ? _chatBuildChoices(node.choices) : _chatResetChoices();
  }
  _chatAddBot(node.answer, extra);
}

function _chatBuildChoices(raw) {
  var choices = (raw || []).map(function(c) {
    var o = { label: c.label, nodeId: c.nodeId, type: 'node' };
    if (c.route) o.route = c.route;
    if (c.goto)  o.goto  = c.goto;
    return o;
  });
  choices.push({ label: '⬅ Back to topics', type: 'reset', secondary: true });
  choices.push({ label: '📞 Book a Strategy Session', cta: true });
  return choices;
}

function _chatResetChoices() {
  return [
    { label: '⬅ Back to topics', type: 'reset', secondary: true },
    { label: '📞 Book a Strategy Session', cta: true }
  ];
}
...
window.adminRenderSubDash = function(container){
  // First call: kick off a Supabase fetch in the background. When it returns,
  // we replace window._SUBMISSIONS and re-invoke this function with fresh data.
  // The _supaFetched flag prevents infinite recursion.
  if(container && !container._supaFetched){
    container._supaFetched = true;
    try {
      fetchSubmissions().then(function(rows){
        container._supaFetched = false; // allow refresh on next open
        if(!rows) return;
        window._SUBMISSIONS = rows;
        // Only re-render if the user is still on the Submissions tab.
        // adminGoHome / other-tab switches set display:none — bailing avoids
        // overwriting the hide and bringing the dashboard back unexpectedly.
        if(container.style.display === 'none') return;
        if(typeof _adminMode !== 'undefined' && _adminMode && _adminMode !== 'submissions') return;
        window.adminRenderSubDash(container);
      }).catch(function(){ container._supaFetched = false; });
    } catch(e){ container._supaFetched = false; }
  }
...
function _subStat(val, label, color){
  return '<div style="text-align:center;min-width:48px;">'
    + '<div style="font-family:Montserrat,sans-serif;font-size:18px;font-weight:900;color:'+(color||'var(--tx)')+';">'+val+'</div>'
  ...
  (function(){
    ...
    PIECES_DATA.forEach(function(pdata, idx){
      ...
      if (pdata.key === 'mirror' && mirrorImgSrc) {
        // Wide logo (~4.4:1 aspect) — give it a wider, vertically-centered box
        // so the wordmark fills more of the puzzle piece.
        logoHTML = '<image class="pp-logo-img" href="' + mirrorImgSrc.replace(/"/g,'&quot;') + '" x="4" y="32" width="92" height="36" preserveAspectRatio="xMidYMid meet"/>';
      } else {
...
var _ZOHO_GROUPS=[
  {cat:"Sales & Marketing",apps:[
    ...
    {key:"bigin",n:"Bigin",note:"Pipeline-centric CRM for small businesses"},
    ...
    {key:"survey",n:"Survey",note:"Surveys + feedback collection"},
    {key:"marketingplus",n:"Marketing Automation",note:"Multi-channel marketing orchestration"},
    {key:"sites",n:"Sites",note:"Website builder"},
    {key:"landingpage",n:"LandingPage",note:"Conversion-focused landing pages"},
    {key:"backstage",n:"Backstage",note:"End-to-end event management"},
    {key:"webinar",n:"Webinar",note:"Webinar + virtual events platform"},
    {key:"pagesense",n:"PageSense",note:"Conversion optimization + heatmaps"},
    {key:"commerce",n:"Commerce",note:"E-commerce storefront platform"},
    {key:"thrive",n:"Thrive",note:"Loyalty + affiliate programs"},
  ]},
  {cat:"Productivity & Collaboration",apps:[
    {key:"mail",n:"Mail",note:"Business email + groupware"},
    {key:"cliq",n:"Cliq",note:"Team chat + channels"},
    {key:"meeting",n:"Meeting",note:"Video meetings + webinars"},
    {key:"workdrive",n:"WorkDrive",note:"File storage + team collaboration"},
    {key:"writer",n:"Writer",note:"Document editor"},
    {key:"sheet",n:"Sheet",note:"Spreadsheets + data tools"},
    {key:"show",n:"Show",note:"Presentation builder"},
    {key:"notebook",n:"Notebook",note:"Notes + idea capture"},
    {key:"connect",n:"Connect",note:"Internal social + intranet"},
    {key:"teaminbox",n:"TeamInbox",note:"Shared email for teams"},
    {key:"bookings",n:"Bookings",note:"Appointment scheduling"},
    {key:"sign",n:"Sign",note:"E-signature + document workflow"},
    {key:"learn",n:"Learn",note:"Learning management system"},
    {key:"lens",n:"Lens",note:"AR remote assistance"},
    {key:"contracts",n:"Contracts",note:"Contract lifecycle management"},
    {key:"vault",n:"Vault",note:"Password + secrets management"},
  ]},
  {cat:"Finance",apps:[
    {key:"books",n:"Books",note:"Accounting + financial operations"},
    {key:"invoice",n:"Invoice",note:"Standalone invoicing"},
    {key:"expense",n:"Expense",note:"Expense + travel management"},
    {key:"inventory",n:"Inventory",note:"Inventory + order management"},
    {key:"billing",n:"Billing",note:"Enterprise subscription billing"},
    {key:"checkout",n:"Checkout",note:"Online payment pages"},
  ]},
  {cat:"HR & Operations",apps:[
    {key:"people",n:"People",note:"HR + workforce management"},
    {key:"recruit",n:"Recruit",note:"Applicant tracking + recruiting"},
    {key:"sprints",n:"Sprints",note:"Agile project management"},
    {key:"projects",n:"Projects",note:"Project + task management"},
    {key:"desk",n:"Desk",note:"Customer support + helpdesk"},
    {key:"assist",n:"Assist",note:"Remote support + screen sharing"},
  ]},
  {cat:"BI & Analytics",apps:[
    {key:"analytics",n:"Analytics",note:"BI + reporting"},
    {key:"dataprep",n:"DataPrep",note:"Data cleaning + transformation"},
  ]},
...
window.adminGoHome = function(){
  ...
  ['adminDashContent','adminSocialContent','adminBgContent',
  ...
  ['tabDashboard','tabSocial','tabBg','tabNav','tabSub','tabSettings'].forEach(function(id){
    var el = document.getElementById(id);
...
window.adminRenderHome = function(container){
  ...
  wrap.innerHTML =
    ...
    + _adminHomeCard('backgrounds', '🖼', 'rgba(76,175,80,.15)',   'Page Backgrounds', 'Set images, GIFs or videos per page')
...
window.adminSelectDash = function(mode){
  ...
  var badges = { cases:'Case Studies', chatbot:'Chatbot', social:'Social Media',
                 backgrounds:'Page Backgrounds', navigation:'Navigation', submissions:'Submissions' };
...
window.adminGoSettings = function(){
  ...
  ['tabDashboard','tabSocial','tabBg','tabNav','tabSub'].forEach(function(id){
    var el = document.getElementById(id);
...
window.adminSwitchMode = function(){
  ...
  var modes = ['cases','chatbot','social','backgrounds','navigation','submissions'];
...
window.adminSetTab = function(tab){
  ...
  ['adminDashContent', 'adminSocialContent', 'adminBgContent', 'adminNavContent', 'adminSubContent', 'adminSettingsContent', 'adminHomeContent'].forEach(function(id){
    var el = document.getElementById(id);
  ...
  if(tab === 'dashboard'){
    ...
    var t = document.getElementById('tabBg');
    var p = document.getElementById('adminBgContent');
    if(t){ t.style.display='flex'; t.classList.add('active'); }
    if(p){ p.style.display='flex'; p.innerHTML=''; adminRenderBgDash(p); }
    ...
    if(t){ t.style.display='flex'; t.classList.add('active'); }
    if(p){ p.style.display='flex'; p.innerHTML=''; adminRenderSubDash(p); }
...
function adminRenderSocialDash(container){
  ...
  var order = ['linkedin','facebook','instagram','x','youtube','google','tiktok'];
...
// ══════════════════════════════════════════════════════════════════════════════
// Module-scoped flag so we fetch latest cases from Supabase exactly once per
// page load. Triggered the first time the Cases dashboard is opened, kicks
// off a background refresh that updates window._CASES + the sidebar — without
// touching the editor pane. Pulling this OUT of adminRenderCasesDash fixes
// the "editor flashes then disappears" bug (the function was being called
// repeatedly by tab switches, and each call kicked off another fetch).
var _casesSupaFetchedOnce = false;
function _casesRefreshFromSupabase(){
  if(_casesSupaFetchedOnce) return;
  _casesSupaFetchedOnce = true;
  var fetchStartedAt = Date.now();
  try {
    fetchConfig('cases').then(function(rows){
      if(!Array.isArray(rows) || rows.length === 0) return;
      // Race guard: if local mutations happened since this fetch started,
      // the in-flight result is stale — ignore.
      if(window._casesLastMutatedAt && window._casesLastMutatedAt >= fetchStartedAt) return;
      window._CASES = rows;
      // Only refresh the sidebar list. NEVER rebuild the dashboard — that
      // would tear down whatever editor the user is currently using.
      if(typeof adminRenderSidebar === 'function') adminRenderSidebar();
      // Also refresh public-facing case grid / home scroll / featured.
      ...
      if(typeof window._renderFeaturedCase === 'function') window._renderFeaturedCase();
    }).catch(function(){ /* swallow — defaults remain in place */ });
  } catch(e){ /* swallow */ }
}

function adminRenderCasesDash(container){
  // Kick off the one-time Supabase refresh in the background. The render
  // proceeds synchronously below using whatever is currently in _CASES.
  _casesRefreshFromSupabase();
  ...
  sidebar.innerHTML =
    ...
    + '<button class="admin-add-btn" onclick="adminNewCase()">+ New Case Study</button>'
    + '</div>';
  ...
  editor.innerHTML =
    ...
    + '<button class="admin-save" onclick="adminSaveCase()">&#10003; Save</button>'
    + '<button class="admin-cancel" onclick="adminCancelEdit()">Cancel</button>'
    + '<button class="admin-hide-btn" id="adminHideBtn" onclick="adminToggleHideCase()" style="display:none;padding:9px 16px;border-radius:8px;border:1px solid rgba(139,159,212,.3);background:rgba(139,159,212,.08);color:#8B9FD4;font-family:Montserrat,sans-serif;font-size:11px;font-weight:700;cursor:pointer;transition:all .2s">&#128065; Hide</button>'
    + '<button class="admin-del-btn" onclick="adminDeleteCase()">Delete</button>'
    + '</div>';
...
function adminRenderSidebar(){
  ...
  list.innerHTML = window._CASES.map(function(c,i){
    ...
    var hiddenStyle = c.hidden ? ' style="opacity:.5"' : '';
    ...
    return '<div class="admin-case-item' + (i===_adminIdx?' active':'') + '"' + hiddenStyle + ' onclick="adminEditCase(' + i + ')">'
      + '<div class="admin-case-item-title">' + c.title + hiddenBadge + '</div>'
      + '<span class="admin-case-item-cat" style="background:' + cols.tc + ';color:' + cols.tt + '">' + c.cat + '</span>'
      + '</div>';
  }).join('');
}
...
function adminGetFormValues(){
  ...
  return {
    ...
    desc:        val('ae-desc'),
    featured:    featured,
    _existingCardMedia:   _adminCardMedia || (existing ? existing.cardMedia   : null),
    _existingDetailMedia: existing ? existing.detailMedia : null,
  };
}

function adminBuildCaseObj(vals){
  ...
  return {
    ...
    blocks:  JSON.parse(JSON.stringify(_adminBlocks)),
...
window.adminLivePreview = function(){
  ...
  if(!pf) return;
  // Always build a complete case object for preview
  var c = adminBuildCaseObj(vals);
  // Preview tab follows the editor tab
...
function _renderDetailPreview(c){
  ...
  var blocks=(_adminBlocks&&_adminBlocks.length)?_adminBlocks:_defaultBlocks(c);
  var html='<div class="cd-hero" style="background:radial-gradient(ellipse at 60% 30%,'+tc+' 0%,transparent 60%),#080B1A;padding:60px 32px 40px">'
...
window.adminEditCase = function(idx){
  ...
  _adminDetailMedia = null;
  _aeActiveTab = 'card';
  _aeCardCache = {};
  _blCollapsed = {};
  adminRenderSidebar();
  adminRenderEditor(window._CASES[idx], false);
};
window.adminNewCase = function(){
  ...
  _blCollapsed = {};
  adminRenderSidebar();
  adminRenderEditor(null, true);
};
window.adminSaveCase = function(){
  var vals = adminGetFormValues();
  console.log('[admin] adminSaveCase called. Form values:', vals, 'editing idx:', _adminIdx);

  // For NEW cases, all required fields must be filled.
  // For EDITS, fall back to the existing case's value for any blank field —
  // so partial edits don't get rejected when only one field is changed.
  if(_adminIdx === -1){
    if(!vals.title || !vals.m1 || !vals.ml1 || !vals.m2 || !vals.ml2 || !vals.desc){
      console.warn('[admin] New-case validation failed. Missing:', {
    ...
    var existing = window._CASES[_adminIdx];
    if(existing){
