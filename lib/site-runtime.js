...
import { saveConfig } from './site-config';

// Fire-and-forget Supabase persistence helper used by admin Save handlers.
// Logs but never throws, so admin UI stays responsive even if the network
// or auth fails.
function _persist(key, value) {
  try {
    saveConfig(key, value).then(function (ok) {
      if (!ok) console.warn('[site-runtime] Supabase save failed:', key);
    });
  } catch (e) {
    console.warn('[site-runtime] Supabase save threw:', key, e);
  }
}
