...
function adminRenderSidebar(){
  // REWRITTEN — uses addEventListener for click binding instead of inline
  // onclick. The previous version could appear to bind correctly while the
  // click still failed to fire (e.g. if an ancestor used capture-phase
  // stopPropagation). Per-item event listeners attached programmatically here
  // are independent and impossible to silently lose.
  ...
  // Build with NO inline onclick (no event-attribute risk at all).
  list.innerHTML = (window._CASES||[]).map(function(c,i){
    ...
    return '<div class="admin-case-item' + (i===_adminIdx?' active':'') + '"' + hiddenStyle + ' data-case-idx="' + i + '" role="button" tabindex="0" style="cursor:pointer">'
      + '<div class="admin-case-item-title">' + _esc(c.title||'') + hiddenBadge + '</div>'
      + '<span class="admin-case-item-cat" style="background:' + cols.tc + ';color:' + cols.tt + '">' + _esc(c.cat||'') + '</span>'
      + '</div>';
  }).join('');

  // Bind click + keyboard handlers per row.
  var items = list.querySelectorAll('.admin-case-item[data-case-idx]');
  items.forEach(function(el){
    var idx = parseInt(el.getAttribute('data-case-idx'), 10);
    el.addEventListener('click', function(e){
      e.preventDefault();
      e.stopPropagation();
      console.log('[admin/cases] sidebar click → idx', idx);
      try {
        if(typeof window.adminEditCase === 'function'){
          window.adminEditCase(idx);
        } else {
          console.error('[admin/cases] window.adminEditCase is not a function!');
        }
      } catch(err){
        console.error('[admin/cases] adminEditCase threw:', err);
      }
    });
  ...
  // BELT-AND-SUSPENDERS: delegated click on the list container. Caught only
  // ONCE per render (we replace the listener via the dataset flag) so it
  // never compounds. Triggers iff per-item handlers somehow missed.
