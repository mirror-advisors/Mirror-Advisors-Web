...
function _chatHandleLeadPayload(payload) {
  ...
  // Persist the captured lead so the SalesIQ `$zoho.salesiq.ready` handler in
  // Layout.js can re-identify this visitor on EVERY future page load. Without
  // this, identification only fires the one time during the chat; if the
  // visitor navigates to another page or returns later, SalesIQ would treat
  // them as anonymous again.
  try {
    if (typeof window !== 'undefined' && window.localStorage) {
      ...
      // Only persist if at least one identifying field is present.
      if (lead.name || lead.email || lead.phone) {
        window.localStorage.setItem('_ma_lead_v1', JSON.stringify(lead));
        console.log('[lead-capture] persisted lead to localStorage._ma_lead_v1:', lead);
      }
    }
  } catch (e) {
  ...
  try {
    console.log('[lead-capture] → step A: calling _chatIdentifyVisitor (SalesIQ JS API)');
...
//
// IMPORTANT API SHAPE NOTE — corrected after the Jun 2026 diagnostic run:
// $zoho.salesiq.ready is NOT a callback-registrar. It is a HOOK that
// SalesIQ INVOKES once during widget init. Calling
// `$zoho.salesiq.ready(fn)` doesn't register `fn` — it invokes the no-op
// stub from zsiq-init with `fn` as an argument, and the stub ignores it.
// The documented pattern (and the one that actually works) is:
//   $zoho.salesiq.ready = function() { ... }
// which means SalesIQ will call our assigned function during init.
//
// But by the time the chat widget completes its lead-capture flow,
// SalesIQ has long since initialized. So we use a two-path strategy:
//
//   Path 1 — DIRECT call: visitor.* setters are available on the global
//            object as soon as the widget is initialized. We call them
//            immediately. This is what actually pushes data to SalesIQ
//            DURING the same session, on the same page.
//
//   Path 2 — Assign $zoho.salesiq.ready as well, in case SalesIQ ever
//            re-runs init (e.g. on cross-tab message, soft reload, or
//            future widget version). Belt-and-braces only.
//
// Why both visitor.info() AND the individual setters:
//   - visitor.info({...}) is the bulk call that fires the proper
//     "profile updated — link to tracking session" event in SalesIQ.
//   - The individual setters (visitor.name/firstname/lastname/email/
//     contactnumber) populate the dedicated dashboard fields. We use
//     the object form of visitor.name({firstname, lastname}) per the
//     SalesIQ docs in addition to the legacy string form.
function _chatIdentifyVisitor(payload) {
  console.log('[lead-capture] _chatIdentifyVisitor entered. window present?', typeof window !== 'undefined',
              '| $zoho present?', !!(typeof window !== 'undefined' && window.$zoho),
              '| $zoho.salesiq present?', !!(typeof window !== 'undefined' && window.$zoho && window.$zoho.salesiq));
  if (typeof window === 'undefined' || !window.$zoho || !window.$zoho.salesiq) {
    console.warn('[lead-capture] $zoho.salesiq UNAVAILABLE — identify aborted. (SalesIQ script may not have loaded.)');
    return;
  }
  ...
  // Internal helper — the actual push. Defined once, called from both
  // Path 1 (immediate) and Path 2 (assigned .ready hook).
  function pushIdentity(label) {
    console.log('[lead-capture] pushIdentity('+label+') firing — sending visitor.* to SalesIQ now.');
    var v = window.$zoho && window.$zoho.salesiq && window.$zoho.salesiq.visitor;
    if (!v) {
      console.warn('[lead-capture] $zoho.salesiq.visitor is UNAVAILABLE in '+label+' — push aborted.');
      return;
    }

    // PRIMARY: bulk info() — fires the "profile updated → link to session" event.
    if (typeof v.info === 'function') {
      ...
      try {
        var r = v.info(bulk);
        console.log('[lead-capture] ['+label+'] ✓ visitor.info(', bulk, ') called. Return value:', r);
      } catch (e) {
        console.warn('[lead-capture] ['+label+'] ✗ visitor.info() THREW:', e);
      }
    } else {
      console.warn('[lead-capture] ['+label+'] visitor.info is NOT a function — skipping bulk call.');
    }

    // Object-form visitor.name({firstname, lastname}) per SalesIQ docs.
    if ((fn || ln) && typeof v.name === 'function') {
      try {
        var rno = v.name({ firstname: fn, lastname: ln });
        console.log('[lead-capture] ['+label+'] ✓ visitor.name({firstname:\''+fn+'\', lastname:\''+ln+'\'}) →', rno);
      } catch (e) { console.warn('[lead-capture] ['+label+'] ✗ visitor.name(obj) THREW:', e); }
    }

    // Individual setters — same data, idempotent. Belt-and-braces for any
    // SalesIQ build that prefers one form over another.
    if (name  && typeof v.name      === 'function') { try { var rn = v.name(name);       console.log('[lead-capture] ['+label+'] ✓ visitor.name(\''+name+'\') →', rn); } catch (e) { console.warn('[lead-capture] ['+label+'] ✗ visitor.name(str) THREW:', e); } }
    if (fn    && typeof v.firstname === 'function') { try { var rf = v.firstname(fn);    console.log('[lead-capture] ['+label+'] ✓ visitor.firstname(\''+fn+'\') →', rf); } catch (e) { console.warn('[lead-capture] ['+label+'] ✗ visitor.firstname THREW:', e); } }
    if (ln    && typeof v.lastname  === 'function') { try { var rl = v.lastname(ln);     console.log('[lead-capture] ['+label+'] ✓ visitor.lastname(\''+ln+'\') →', rl); } catch (e) { console.warn('[lead-capture] ['+label+'] ✗ visitor.lastname THREW:', e); } }
    if (email && typeof v.email     === 'function') { try { var re_ = v.email(email);    console.log('[lead-capture] ['+label+'] ✓ visitor.email(\''+email+'\') →', re_); } catch (e) { console.warn('[lead-capture] ['+label+'] ✗ visitor.email THREW:', e); } }
    if (phone && typeof v.contactnumber === 'function') { try { var rp = v.contactnumber(phone); console.log('[lead-capture] ['+label+'] ✓ visitor.contactnumber(\''+phone+'\') →', rp); } catch (e) { console.warn('[lead-capture] ['+label+'] ✗ visitor.contactnumber THREW:', e); } }

    // Log the current siqid so the link can be verified against the
    // SalesIQ dashboard (search for this id or its URL-decoded form).
    try {
      var siqid = window.$zoho.salesiq.tracking
               && typeof window.$zoho.salesiq.tracking.getsiqid === 'function'
               ? window.$zoho.salesiq.tracking.getsiqid()
               : '(no tracking.getsiqid available)';
      console.log('[lead-capture] ['+label+'] identify COMPLETE for siqid:', siqid);
    } catch (_) {}
  }

  // PATH 1 — direct push, runs RIGHT NOW. By the time the chat completes,
  // SalesIQ has long since initialized and visitor.* is already available.
  try {
    pushIdentity('direct');
  } catch (e) {
    console.warn('[lead-capture] direct push THREW:', e);
  }

  // PATH 2 — assign $zoho.salesiq.ready per the SalesIQ docs pattern,
  // in case the widget ever re-runs its init hook. This is the same form
  // used by the zsiq-identify Script in Layout.js for next-page-load
  // re-identification. Idempotent — calling pushIdentity twice is safe.
  try {
    window.$zoho.salesiq.ready = function () {
      console.log('[lead-capture] $zoho.salesiq.ready hook INVOKED by SalesIQ init.');
      pushIdentity('ready-hook');
    };
    console.log('[lead-capture] $zoho.salesiq.ready hook ASSIGNED. Will fire if SalesIQ re-inits.');
  } catch (e) {
    console.warn('[lead-capture] assigning $zoho.salesiq.ready THREW:', e);
  }
}
