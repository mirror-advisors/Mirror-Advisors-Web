...
// ── NAV RENDERER ─────────────────────────────────────────────────────────────
// Sub-links surfaced under the Services hover-dropdown. Kept as data so
// both this render path and the JSX in components/Layout.js stay in sync
// visually — same items, same order, same routes.
var _SERVICES_SUBLINKS = [
  { route: 'services',                       label: 'Overview' },
  { route: 'services/zoho-implementation',   label: 'Zoho Implementation' },
  { route: 'services/digital-marketing',     label: 'Digital Marketing' },
  { route: 'services/custom-ai-application', label: 'Custom AI Application' },
];
...
function _renderNav(){
  ...
  if(nl){
    ...
    nl.innerHTML = pages.filter(function(p){ return p.enabled; }).map(function(p){
      ...
      if (p.key === 'services') {
        var subs = _SERVICES_SUBLINKS.map(function(s){
          return '<a href="#" onclick="go(\'' + s.route + '\'); return false;" class="nav-drop-link">' + s.label + '</a>';
        }).join('');
