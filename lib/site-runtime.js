...
function adminRenderCasesDash(container){
  // First-open: fetch latest cases from Supabase so the editor reflects any
  // edits made from another device. Re-renders with fresh data on completion;
  // bails if the user navigated away OR if a local save/delete happened while
  // the fetch was in flight (otherwise stale data overwrites the new state).
  if(container && !container._supaFetched){
    container._supaFetched = true;
    ...
    try {
      fetchConfig('cases').then(function(rows){
        ...
        // Race guard: if any local mutation (save/delete/new) happened since
        // this fetch started, the in-flight result is stale — ignore it.
        if(window._casesLastMutatedAt && window._casesLastMutatedAt >= fetchStartedAt) return;
        window._CASES = rows;
        if(container.style.display === 'none') return;
        if(typeof _adminMode !== 'undefined' && _adminMode && _adminMode !== 'cases') return;
        // If the user is currently editing a case, only refresh the sidebar
        // (the case list). Re-rendering the whole dashboard would tear down
        // their editor mid-typing — that's the "preview shows then disappears"
        // bug. When not in a case, it's safe to rebuild the full dashboard.
        if(typeof _adminIdx !== 'undefined' && _adminIdx >= 0){
          if(typeof adminRenderSidebar === 'function') adminRenderSidebar();
        } else {
          adminRenderCasesDash(container);
        }
      }).catch(function(){ container._supaFetched = false; });
    } catch(e){ container._supaFetched = false; }
  }
