...
// Legacy: kept as a stub for backwards-compat in case any other markup still
// references onsubmit="submitForm(...)". The new contact form is bound by
// _INIT.contact and posts straight to Zoho Forms via a hidden iframe.
window.submitForm = function(e){
  if(e && e.preventDefault) e.preventDefault();
  console.warn('[contact] submitForm() is deprecated. The contact form now binds via _INIT.contact and POSTs to Zoho Forms.');
};
...
_INIT.contact=function(){
  // Bind the Zoho-targeted contact form. Runs on every navigation TO /contact;
  // a dataset flag on the form prevents double-binding when the same DOM node
  // is re-rendered (Next.js page transition doesn't always destroy it).
  var form = document.getElementById('contactForm');
  if(!form || form.dataset.cfBound === '1') return;
  ...
  var SIZE     = document.getElementById('cf-size');
  var TIMELINE = document.getElementById('cf-timeline');
  var IFRAME   = document.getElementById('zohoFormFrame');
  var REFERRER = form.querySelector('input[name="zf_referrer_name"]');

  // Stamp the referring page so Zoho's CRM record shows where the lead came from.
  if(REFERRER){
    try { REFERRER.value = document.referrer || window.location.href; } catch(_){}
  }
  ...
  function setErr(input, msg){
    ...
    var slot = form.querySelector('.cf-err[data-for="' + input.id + '"]');
  ...
  [FNAME, EMAIL, MESSAGE].forEach(function(el){
    if(el) el.addEventListener('input', function(){ clearErr(el); });
  });

  function validate(){
    var ok = true;
    if(FNAME && !FNAME.value.trim()){ setErr(FNAME, 'First name is required.'); ok = false; }
    if(EMAIL){
      var v = EMAIL.value.trim();
      var re = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
      if(!v){ setErr(EMAIL, 'Email is required.'); ok = false; }
      else if(!re.test(v)){ setErr(EMAIL, 'Please enter a valid email address.'); ok = false; }
    }
    if(MESSAGE && !MESSAGE.value.trim()){ setErr(MESSAGE, 'Tell us a bit about what you need.'); ok = false; }
    return ok;
  }

  // ── Merge UI-only fields into MultiLine before submit ──────────────────────
  // The Zoho form schema only has First/Last/Email/Phone/MultiLine. The
  // optional Company/Size/Services/Timeline UI inputs are stitched into
  // MultiLine as a structured prefix so they reach the CRM record and the
  // notification email without needing additional Zoho fields.
  function packMultiLine(){
    if(!MESSAGE) return;
    var raw = MESSAGE.value.trim();
    var prefix = [];
    if(COMPANY && COMPANY.value.trim())  prefix.push('Company: '  + COMPANY.value.trim());
    if(SIZE && SIZE.value.trim())        prefix.push('Size: '     + SIZE.value.trim());
    var chips = form.querySelectorAll('#svcChips .svc-chip.active, #svcChips .svc-chip.sel');
    if(chips.length){
      var svcs = Array.prototype.map.call(chips, function(c){ return c.textContent.trim(); });
      prefix.push('Services: ' + svcs.join(', '));
    }
    if(TIMELINE && TIMELINE.value.trim()) prefix.push('Timeline: ' + TIMELINE.value.trim());

    // Only mutate the textarea value when we actually have prefix lines to
    // add — otherwise we'd send the form with a value that includes empty
    // structure even when the user typed nothing extra. The value is restored
    // after the submit fires so the visible UI isn't disrupted.
    if(prefix.length){
      MESSAGE.dataset.cfOriginal = raw;
      MESSAGE.value = prefix.join('\n') + '\n\nMessage:\n' + raw;
    }
  }
  function unpackMultiLine(){
    if(MESSAGE && MESSAGE.dataset.cfOriginal !== undefined){
      MESSAGE.value = MESSAGE.dataset.cfOriginal;
      delete MESSAGE.dataset.cfOriginal;
    }
  }
  ...
  form.addEventListener('submit', function(e){
    ...
    if(btn){ btn.dataset.cfOriginalText = btn.innerHTML; btn.innerHTML = 'Sending&hellip;'; btn.style.opacity = '.7'; btn.disabled = true; }

    // Parallel write to Supabase so the admin Submissions dashboard still
    // sees this lead. Zoho handles the email notification on its end.
    try {
      var chipsActive = form.querySelectorAll('#svcChips .svc-chip.active, #svcChips .svc-chip.sel');
      var sub = {
        id:        Date.now(),
        datetime:  new Date().toLocaleString(),
        name:      ((FNAME && FNAME.value.trim()) + ' ' + (document.getElementById('cf-lname') ? document.getElementById('cf-lname').value.trim() : '')).trim(),
        email:     EMAIL ? EMAIL.value.trim() : '',
        company:   COMPANY ? COMPANY.value.trim() : '',
        size:      SIZE ? SIZE.value.trim() : '',
        timeline:  TIMELINE ? TIMELINE.value.trim() : '',
        services:  Array.prototype.map.call(chipsActive, function(c){ return c.textContent.trim(); }),
        message:   (MESSAGE && MESSAGE.dataset.cfOriginal) || (MESSAGE && MESSAGE.value) || '',
        status:    'New',
        contacted: false,
      };
      window._SUBMISSIONS = window._SUBMISSIONS || [];
      window._SUBMISSIONS.unshift(sub);
      if(typeof insertSubmission === 'function'){
        insertSubmission(sub).catch(function(){ /* swallow — Zoho is the primary path */ });
      }
    } catch(_){ /* never block submit */ }

    // Restore the textarea value after the form has actually serialised it.
    // setTimeout(0) lets the browser POST the packed value first.
    setTimeout(unpackMultiLine, 0);
  });

  // ── Success detection via the hidden iframe's load event ───────────────────
  // The iframe loads ONCE on mount (initial blank document) — we ignore that.
  // After the form submits into the iframe, its load event fires again; that's
  // when we swap to the success state.
  if(IFRAME){
