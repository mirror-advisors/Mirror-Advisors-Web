...
import { registerLead } from './salesiq';
...
export function initSiteRuntime(router) {
  ...
  window._highlightNav         = _highlightNav;
...
// ── NAV RENDERER ─────────────────────────────────────────────────────────────
// Sub-links surfaced under the Services hover-dropdown. Each entry ties a
// route to a _NAV_PAGES key so the admin toggle can hide the individual
// sub-page. The Overview link (route: 'services') maps to the same key as
// the parent — it always appears when the Services parent is enabled.
var _SERVICES_SUBLINKS = [
  { route: 'services',                        key: 'services',             label: 'Overview' },
  { route: 'services/systems-integration',    key: 'systems-integration',  label: 'Systems Integration' },
  { route: 'services/data-migration',         key: 'data-migration',       label: 'Data Migration' },
  { route: 'services/erp-implementation',     key: 'erp-implementation',   label: 'ERP Implementation' },
  { route: 'services/custom-development',     key: 'custom-development',   label: 'Custom Development' },
  { route: 'services/ai-automation',          key: 'ai-automation',        label: 'AI & Automation', featured: true },
  { route: 'services/zoho',                   key: 'zoho',                 label: 'Zoho Consulting & Support' },
];
...
function _renderNav(){
  ...
  if(nl){
    ...
    // Build a {key: page} lookup so the Services dropdown can cross-check
    // each sub-link against its enabled flag + edited label.
    var byKey = {};
    pages.forEach(function(p){ if(p && p.key) byKey[p.key] = p; });

    // Only top-level pages render as their own nav item. Entries with a
    // `parent` field surface inside their parent's dropdown instead.
    nl.innerHTML = pages.filter(function(p){ return p.enabled && !p.parent; }).map(function(p){
      // Services item is special-cased as a hover-dropdown. Wraps the
      // usual link in a .nav-drop and emits the sub-links menu inline so
      // this render pass (which replaces the whole innerHTML) can't wipe
      // the dropdown structure the way it did with the JSX version.
      if (p.key === 'services') {
        var subs = _SERVICES_SUBLINKS.filter(function(s){
          // Overview link (key === 'services') follows the parent's
          // enabled flag (which is always true here since we already
          // filtered to p.enabled). Every other sub-link is toggleable
          // via _NAV_PAGES; default to visible if the key isn't present.
          if (s.key === 'services') return true;
          var info = byKey[s.key];
          return !info || info.enabled;
        }).map(function(s){
          var info = byKey[s.key];
          var label = (info && info.label) || s.label;
          var cls = 'nav-drop-link' + (s.featured ? ' nav-drop-link-featured' : '');
          return '<a href="#" onclick="go(\'' + s.route + '\'); return false;" class="' + cls + '">' + label + '</a>';
        }).join('');
        return '<div class="nav-drop">' +
          '<a href="#" onclick="go(\'services\'); return false;">' + p.label + '</a>' +
          '<div class="nav-drop-menu" role="menu" aria-label="Services">' + subs + '</div>' +
        '</div>';
      }
...
window.submitForm = function(e){
  ...
  function clearAllErrors(){
    ['fname','lname','email','phone','company','size','svcChips','message','timeline','cfTurnstile'].forEach(clearErr);
    var fe = document.getElementById('formErr');
    if (fe) { fe.style.display = 'none'; fe.textContent = ''; }
  }
  // Reset the Turnstile widget so the visitor gets a fresh challenge to
  // retry after a failed submission. Turnstile tokens are single-use; a
  // failed /api/contact call consumes the token, so the widget must be
  // reissued before the next submit attempt.
  ...
  // Company size is OPTIONAL. Only flag an error if the visitor TYPED something
  // that isn't a positive integer; blank is fine. Mirrors the same rule in
  ...
  // Cloudflare Turnstile — grab the token the widget wrote into its
  // hidden input. If the visitor hasn't solved the challenge yet the
  // input is empty; block submission until they do.
  var turnstileInput = document.querySelector('input[name="cf-turnstile-response"]');
  var turnstileToken = turnstileInput ? String(turnstileInput.value || '').trim() : '';
  if (!turnstileToken) {
    setErr('cfTurnstile', 'Please complete the verification challenge.');
    problems.push('cfTurnstile');
  }
  ...
  // Honeypot — invisible field bots fill blindly. Real users can't
  // interact with it (display:none + tabindex="-1"), so any value here
  // is a strong bot signal. Forwarded to the API which returns 200 OK
  // silently on match, so the bot walks away thinking it succeeded.
  var website_url = val('website_url');
  ...
  fetch('/api/contact', {
    ...
    .then(function (res) {
      ...
      // Turnstile token was consumed by the failed attempt; give the
      // visitor a fresh challenge so their retry can succeed.
      resetTurnstile();
    })
    .catch(function () {
      var fe = document.getElementById('formErr');
...
// Heuristic: does this string plausibly look like a real human name?
// The lead-capture flow can persist whatever the visitor typed, including
// keyboard mash like "bkjgcrtxckuhb" or single-letter repeats — we don't
// want those surfacing in a "Welcome back, …" greeting. Applied at read
// time (below) AND write time (_chatHandleLeadPayload) so garbage doesn't
// get saved in the first place going forward.
function _chatLooksLikeRealName(s) {
  var name = String(s || '').trim();
  if (name.length < 2 || name.length > 40) return false;
  // Letters, spaces, hyphens, apostrophes only. Rejects digits, symbols, URLs.
  if (!/^[A-Za-z\s'\-]+$/.test(name)) return false;
  // Must contain at least one vowel — rules out consonant-only mashes.
  if (!/[aeiouAEIOU]/.test(name)) return false;
  // Reject strings dominated by one letter, e.g. "aaaa" or "asdasdasd".
  var letters = name.toLowerCase().replace(/[^a-z]/g, '');
  if (!letters.length) return false;
  var seen = {};
  var maxCount = 0;
  for (var i = 0; i < letters.length; i++) {
    seen[letters[i]] = (seen[letters[i]] || 0) + 1;
    if (seen[letters[i]] > maxCount) maxCount = seen[letters[i]];
  }
  if (maxCount / letters.length > 0.7) return false;
  // Reject long runs of consecutive consonants — classic keyboard-mash sign.
  if (/[^aeiouAEIOU\s'\-]{6,}/.test(name)) return false;
  return true;
}

// If the visitor previously handed us their name (persisted via the
// lead-capture flow to _ma_lead_v1), return the first name. Otherwise ''.
// Guards against garbage in localStorage via _chatLooksLikeRealName so a
// bogus stored value silently falls back to the default greeting.
function _chatReturningFirstName() {
  try {
    var raw = localStorage.getItem('_ma_lead_v1');
    if (!raw) return '';
    var lead = JSON.parse(raw);
    var full = (lead && typeof lead.name === 'string') ? lead.name.trim() : '';
    if (!_chatLooksLikeRealName(full)) return '';
    // First whitespace-separated token, stripped of punctuation.
    var first = full.split(/\s+/)[0].replace(/[^A-Za-z'\-]/g, '');
    if (!_chatLooksLikeRealName(first)) return '';
    return first;
  } catch (_) {
    return '';
  }
...
function _chatStart() {
  ...
  // If we know the visitor's name from a prior session, swap in a warmer
  // welcome-back greeting; otherwise use the default introducer.
  var firstName = _chatReturningFirstName();
  var greeting = firstName
    ? ('Welcome back, ' + firstName + ' — good to see you again. What would you like to explore today?')
    : _CHAT_GREETING;
  _chatRenderBot(greeting);
...
// ── Fresh-conversation-per-page-load ─────────────────────────────────
// On every page load we clear the "current conversation" pointer so
// _chatRestoreCurrent() bails out and the widget opens with a fresh
// greeting + starter row. Past conversations are NOT wiped — they stay
// in localStorage and remain browseable via the widget's history list.
// Only auto-restore is disabled.
(function _chatBootResetCurrent() {
  try {
    var raw = localStorage.getItem(_CHAT_STORE_KEY);
    if (!raw) return;
    var parsed = JSON.parse(raw);
    if (!parsed || typeof parsed !== 'object') return;
    if (parsed.currentId === null) return;   // already fresh
    parsed.currentId = null;
    localStorage.setItem(_CHAT_STORE_KEY, JSON.stringify(parsed));
  } catch (_) {
    // Private mode / quota / bad JSON — silently drop; chat still works.
  }
})();
...
// VERBOSE LOGGING enabled — every reply is announced with its length and
// outcome so you can confirm in DevTools console whether (a) the AI is
// emitting the block, (b) the regex matches, and (c) the JSON parses.
function _chatExtractLeadBlock(text) {
  if (!text || typeof text !== 'string') {
    console.log('[lead-capture] extractor called with empty/non-string text');
    return { cleanedText: text || '', payload: null };
  }
  console.log('[lead-capture] scanning bot reply (length =', text.length + ' chars)');
  var re = /<<<MA_LEAD>>>([\s\S]*?)<<<MA_END>>>/;
  var m = text.match(re);
  if (!m) {
    console.log('[lead-capture] no <<<MA_LEAD>>>...<<<MA_END>>> block in this reply');
    return { cleanedText: text, payload: null };
  }
  var rawJson = m[1].trim();
  console.log('[lead-capture] block FOUND. Raw JSON between delimiters:', rawJson);
  var cleanedText = text.replace(/<<<MA_LEAD>>>[\s\S]*?<<<MA_END>>>/g, '').trim();
  var payload = null;
  try {
    payload = JSON.parse(rawJson);
    console.log('[lead-capture] JSON.parse SUCCEEDED. Parsed payload:', payload);
  } catch (e) {
    console.warn('[lead-capture] JSON.parse FAILED:', e && e.message,
...
// VERBOSE LOGGING enabled — see [lead-capture] entries in DevTools console
// to confirm each branch was entered and its outcome.
function _chatHandleLeadPayload(payload) {
  if (!payload || typeof payload !== 'object' || !payload.action) {
    console.warn('[lead-capture] handler called with invalid payload:', payload);
    return;
  }
  console.log('[lead-capture] handler entered. action =', payload.action,
              '| name=', payload.name, '| email=', payload.email,
              '| phone=', payload.phone,
              (payload.action === 'lead' ? '| message='+(payload.message || '').slice(0,80)+'…' : ''));

  // Persist the captured lead so the <SalesIQ /> init script can replay
  // it into visitor.* on every subsequent page load. Without this, the
  // identification only lands on the page where the chat happened —
  // refreshes, new tabs, and return visits would show as anonymous.
  try {
    if (typeof window !== 'undefined' && window.localStorage) {
      // Guard: only persist a name that plausibly looks like one. Keyboard
      // mash ("bkjgcrtxckuhb"), single-letter repeats, digits, URLs, etc.
      // get dropped so the personalized "Welcome back, X" greeting never
      // surfaces garbage. Email and phone are validated by their own
      // upstream capture prompts, so we trust them as-is.
      var rawName = String(payload.name || '').trim();
      var cleanName = _chatLooksLikeRealName(rawName) ? rawName : '';
      if (rawName && !cleanName) {
        console.warn('[lead-capture] dropped bogus-looking name from persist:', JSON.stringify(rawName));
      }
      var persisted = {
        name:  cleanName,
      ...
      if (persisted.name || persisted.email || persisted.phone) {
        window.localStorage.setItem('_ma_lead_v1', JSON.stringify(persisted));
        console.log('[lead-capture] persisted lead to localStorage._ma_lead_v1:', persisted);
      }
    }
  } catch (e) {
    console.warn('[lead-capture] failed to persist lead to localStorage:', e);
  }

  try {
    console.log('[lead-capture] → step A: registerLead → SalesIQ JS API (queues if widget not yet ready).');
    _chatIdentifyVisitor(payload);
  } catch (e) {
    console.warn('[lead-capture] _chatIdentifyVisitor THREW:', e);
  }
  if (payload.action === 'lead') {
    try {
      console.log('[lead-capture] → step B: calling _chatSubmitLead (POST /api/contact → Zoho CRM)');
      _chatSubmitLead(payload).then(function (res) {
        if (!res || !res.ok) {
          console.warn('[lead-capture] /api/contact returned non-ok:', res);
        } else {
          console.log('[lead-capture] /api/contact OK. response.data:', res.data);
        }
      });
    } catch (e) {
      console.warn('[lead-capture] _chatSubmitLead THREW:', e);
    }
  } else {
    console.log('[lead-capture] action ≠ "lead" — skipping /api/contact submit (correct for action=identify)');
  }

  // Auto-close the chat panel a couple of seconds after the lead-capture
  // flow completes (End conversation OR Leave a message). The brief delay
  // lets the user read the AI's confirmation message before the panel
  // slides away. Reuses window.chatToggle so the launcher button + pulse
  // animation update in lockstep with the panel — same path a manual close
  // takes. Guarded on _chatOpen so we don't toggle the panel back open if
  // the user already closed it manually during the delay.
  try {
    setTimeout(function () {
      if (_chatOpen && typeof window.chatToggle === 'function') {
        console.log('[lead-capture] auto-closing chat panel (lead-capture flow complete).');
        window.chatToggle();
      } else {
        console.log('[lead-capture] auto-close skipped — chat already closed or chatToggle unavailable.');
      }
    }, 2500);
  } catch (e) {
    console.warn('[lead-capture] auto-close scheduling threw:', e);
  }
}

// Push the lead the chat widget just collected into SalesIQ. The actual
// JS-API work lives in lib/salesiq.js → registerLead(), which handles SSR
// guards, the queue (calls before SalesIQ ready get replayed), and the
// individual visitor.* setters. This wrapper only adds chat-side
// diagnostic logging so the existing [lead-capture] timeline stays
// readable in DevTools.
function _chatIdentifyVisitor(payload) {
  console.log('[lead-capture] _chatIdentifyVisitor entered.',
  ...
  try {
    ...
    console.log('[lead-capture] registerLead dispatched. (Queues if SalesIQ not yet ready; runs immediately if it is.)');
  } catch (e) {
    console.warn('[lead-capture] registerLead THREW:', e);
  }
}
...
function _chatSubmitLead(payload) {
  ...
  var body = {
    ...
    company:  '(via chatbot)',
    size:     '',
    services: ['Zoho implementation'],
    timeline: 'Just exploring',
  };

  console.log('[lead-capture] POSTing to /api/contact with payload:', body);
  return fetch('/api/contact', {
    ...
    .then(function (r) {
      console.log('[lead-capture] /api/contact response received. HTTP status =', r.status);
      return r.json().then(function (d) {
        console.log('[lead-capture] /api/contact response body:', d);
        return { ok: r.ok, data: d };
      }, function () {
        console.warn('[lead-capture] /api/contact returned non-JSON body');
        return { ok: r.ok, data: {} };
      });
    })
    .catch(function (e) {
      console.warn('[lead-capture] /api/contact fetch THREW:', e);
...
function _navRenderRows(pages){
  return pages.map(function(p, i){
    ...
    // Sub-pages (entries with a parent field) render with a small indent
    // + a "↳" glyph in place of the row number so admins can see the
    // hierarchy at a glance.
    var isSub = !!p.parent;
    return '<div class="nav-page-row' + (p.enabled?'':' disabled') + (isSub?' nav-page-row-sub':'') + '" id="navRow_'+i+'"'
      + ' draggable="true"'
      ...
      + '<span class="nav-page-num">' + (isSub ? '&#x21B3;' : (i+1)) + '</span>'
...
window.navResetDefaults = function(){
  window._NAV_PAGES = [
    ...
    { key:'systems-integration',  label:'Systems Integration',       enabled:true, parent:'services' },
    { key:'data-migration',       label:'Data Migration',            enabled:true, parent:'services' },
    { key:'erp-implementation',   label:'ERP Implementation',        enabled:true, parent:'services' },
    { key:'custom-development',   label:'Custom Development',        enabled:true, parent:'services' },
    { key:'ai-automation',        label:'AI & Automation',           enabled:true, parent:'services' },
    { key:'zoho',                 label:'Zoho Consulting & Support', enabled:true, parent:'services' },
    { key:'how-we-work',          label:'How We Work',               enabled:true },
...
// Service options for the Submissions admin filter + edit panel. Mirror
// the contact form (pages/api/contact.js → ALLOWED_SERVICES) so new
// submissions surface under the right filter chip.
var _SUB_SERVICES = ['Zoho implementation','Custom AI Application'];
...
window.subConfirmDelete = function(id){
  ...
  // Refresh whichever list is currently mounted. Both dashboards read from
  // the shared window._SUBMISSIONS array, so calling both is safe \u2014 the one
  // whose DOM isn't visible just no-ops.
  if(typeof subRenderList === 'function') { try { subRenderList(); } catch(_){} }
  if(typeof aiSignupAdminRenderList === 'function') { try { aiSignupAdminRenderList(); } catch(_){} }
  if(typeof _subUpdateStats === 'function') { try { _subUpdateStats(); } catch(_){} }
  if(typeof _aiSignupUpdateStats === 'function') { try { _aiSignupUpdateStats(); } catch(_){} }
...
// ══════════════════════════════════════════════════════════════════════════════
// AI SIGNUPS DASHBOARD
// AI free-chatbot-session requests live in the same Supabase `submissions`
// table as contact-form leads, tagged with data.source = 'ai-signup'. This
// dashboard filters that table to just those rows and renders them with the
// AI-specific fields (motivation, website, tech stack, contact time) instead
// of the contact-form fields.
// ══════════════════════════════════════════════════════════════════════════════

var _aiSignupFilter   = { status:'all', contacted:'all' };
var _aiSignupExpanded = {};

function _aiSignupRows(){
  return (window._SUBMISSIONS || []).filter(function(s){ return s.source === 'ai-signup'; });
}

function _aiSignupGetFiltered(){
  return _aiSignupRows().filter(function(s){
    if(_aiSignupFilter.status !== 'all' && s.status !== _aiSignupFilter.status) return false;
    if(_aiSignupFilter.contacted === 'yes' && !s.contacted) return false;
    if(_aiSignupFilter.contacted === 'no'  &&  s.contacted) return false;
    return true;
  });
}

window.adminRenderAiSignupsDash = function(container){
  // Same Supabase fetch guard pattern as adminRenderSubDash: one fetch per
  // dashboard mount, in-place refresh so scroll position is preserved.
  if(container && !container._supaFetched){
    container._supaFetched = true;
    try {
      fetchSubmissions().then(function(rows){
        ...
        window._SUBMISSIONS = rows;
        if(container.style.display === 'none') return;
        if(typeof _adminMode !== 'undefined' && _adminMode && _adminMode !== 'ai-signups') return;
        if(typeof aiSignupAdminRenderList === 'function') aiSignupAdminRenderList();
        if(typeof _aiSignupUpdateStats === 'function') _aiSignupUpdateStats();
      }).catch(function(){});
    } catch(e){ /* swallow */ }
  }
  ...
  toolbar.innerHTML =
    '<div style="display:flex;align-items:center;gap:18px;flex-wrap:wrap;">'
    + '<div style="display:flex;gap:18px;flex-shrink:0;">'
    + _subStat(rows.length, 'Total')
    + _subStat(rows.filter(function(s){return s.status==='New';}).length, 'New', '#ECA934')
    + _subStat(rows.filter(function(s){return !s.contacted;}).length, 'Pending', 'rgba(255,80,80,.8)')
    + '</div>'
    + '<div style="flex:1;min-width:0;"></div>'
    + '<span class="sub-count" id="aiSignupCount" style="font-size:11px;color:var(--dim);font-family:Montserrat,sans-serif;font-weight:700;">'+rows.length+' signup'+(rows.length===1?'':'s')+'</span>'
    + '<button class="admin-save" style="padding:7px 14px;font-size:11px;flex-shrink:0;" onclick="aiSignupDownloadPDF()">⬇ Export PDF</button>'
    + '</div>'
    + '<div style="display:flex;gap:18px;flex-wrap:wrap;align-items:center;">'
    + '<div style="display:flex;align-items:center;gap:6px;">'
    + '<span style="font-size:9px;color:var(--dim);font-family:Montserrat,sans-serif;font-weight:800;text-transform:uppercase;letter-spacing:.08em;">Status:</span>'
    + '<div id="aiSignupStatusFilters" style="display:flex;gap:4px;flex-wrap:wrap;">'
    + ['All','New','Read','Replied'].map(function(s){
        return '<button class="sub-filter-pill'+(s==='All'?' active':'')+'" onclick="aiSignupFilterStatus(\''+s+'\')">'+s+'</button>';
      }).join('')
    + '</div></div>'
    + '<div style="display:flex;align-items:center;gap:6px;">'
    + '<span style="font-size:9px;color:var(--dim);font-family:Montserrat,sans-serif;font-weight:800;text-transform:uppercase;letter-spacing:.08em;">Contact:</span>'
    + '<div id="aiSignupContactedFilters" style="display:flex;gap:4px;">'
    + '<button class="sub-filter-pill active" onclick="aiSignupFilterContacted(\'all\')">All</button>'
    + '<button class="sub-filter-pill" onclick="aiSignupFilterContacted(\'yes\')">✓ Done</button>'
    + '<button class="sub-filter-pill" onclick="aiSignupFilterContacted(\'no\')">Pending</button>'
    + '</div></div>'
    + '</div>';

  // List
  var list = document.createElement('div');
  list.id = 'aiSignupList';
  list.style.cssText = 'flex:1;overflow-y:auto;min-height:0;padding:16px 20px;display:flex;flex-direction:column;gap:10px;';

  container.appendChild(toolbar);
  container.appendChild(list);
  aiSignupAdminRenderList();
};

function _aiSignupUpdateStats(){
  var rows = _aiSignupRows();
  function setStat(label, val){
    var content = document.getElementById('adminAiSignupsContent');
    if(!content) return;
    var cell = content.querySelector('[data-stat="' + label + '"] .sub-stat-val');
    if(cell) cell.textContent = val;
  }
  setStat('Total',   rows.length);
  setStat('New',     rows.filter(function(s){ return s.status === 'New'; }).length);
  setStat('Pending', rows.filter(function(s){ return !s.contacted; }).length);
}

function aiSignupAdminRenderList(){
  var list = document.getElementById('aiSignupList');
  if(!list) return;
  var filtered = _aiSignupGetFiltered();
  var countEl = document.getElementById('aiSignupCount');
  if(countEl) countEl.textContent = filtered.length + ' signup' + (filtered.length===1?'':'s');

  if(!filtered.length){
    list.innerHTML = '<div class="sub-empty"><div class="sub-empty-icon">🤖</div>'
      + (_aiSignupRows().length
          ? 'No AI signups match the current filters.'
          : 'No AI signups yet.<br>They will appear here when clients fill out the sign-up form on the AI page.')
      + '</div>';
    return;
  }
  ...
  list.innerHTML = filtered.map(function(s){
    var statusClass  = 'status-' + (s.status||'new').toLowerCase();
    var statusBadge  = 'sub-status-' + (s.status||'new').toLowerCase();
    var isExp        = _aiSignupExpanded[s.id];
    var motivation   = Array.isArray(s.services) ? s.services : [];
    var motiChips    = motivation.map(function(m){
      return '<span class="sub-svc-tag">'+_esc(m)+'</span>';
    }).join('');

    // Build detail rows for AI signup fields
    var details = '';
    if(s.email) details += _subField('Business Email', s.email);
    if(s.phone) details += _subField('Phone',          s.phone);

    return '<div class="sub-card '+statusClass+'" id="aiSignupCard_'+s.id+'">'
      + '<div class="sub-card-header">'
      + '<div><div class="sub-card-name">'+_esc(s.name||'(no name)')+'</div>'
      + '<div class="sub-card-email">Submitted '+_esc(s.datetime||'')+'</div></div>'
      + '<div class="sub-card-meta">'
      + '<span class="sub-status-badge '+statusBadge+'">'+_esc(s.status||'New')+'</span>'
      + '</div></div>'

      + (details ? '<div class="sub-details-grid">'+details+'</div>' : '')

      + (motiChips
          ? '<div class="sub-field-label">Motivation</div>'
            + '<div class="sub-services" style="margin-bottom:10px">'+motiChips+'</div>'
          : '')

      + '<div class="sub-field-label">Details</div>'
      + '<div class="sub-message'+(isExp?' expanded':'')+'">'+_esc(s.message||'—')+'</div>'
      + (s.message && s.message.length > 120
          ? '<button class="sub-action-btn" style="margin-bottom:10px;margin-top:4px" onclick="aiSignupAdminToggleMsg('+s.id+')">'+(isExp?'Show less ▲':'Show more ▼')+'</button>'
          : '')

      + '<div class="sub-card-footer">'
      + '<div class="sub-contacted-row'+(s.contacted?' contacted':'')+'" id="aiSignupCon_'+s.id+'">'
      + '<button class="social-toggle '+(s.contacted?'on':'')+'" id="aiSignupConBtn_'+s.id+'" onclick="aiSignupAdminToggleContacted('+s.id+')" title="Contacted back"></button>'
      + '<span style="font-size:11px">'+(s.contacted?'✓ Contacted Back':'Not Contacted Yet')+'</span>'
      + '</div>'
      + '<div style="margin-left:auto;display:flex;gap:6px;align-items:center">'
      + '<select class="sub-action-btn" onchange="aiSignupAdminChangeStatus('+s.id+',this.value)" style="cursor:pointer">'
      + ['New','Read','Replied'].map(function(st){
          return '<option'+(s.status===st?' selected':'')+'>'+st+'</option>';
        }).join('')
      + '</select>'
...
window.aiSignupFilterStatus = function(val){
  _aiSignupFilter.status = val;
  document.querySelectorAll('#aiSignupStatusFilters .sub-filter-pill').forEach(function(b){
    b.classList.toggle('active', b.textContent.trim().toLowerCase() === val.toLowerCase());
  });
  aiSignupAdminRenderList();
};

window.aiSignupFilterContacted = function(val){
  _aiSignupFilter.contacted = val;
  document.querySelectorAll('#aiSignupContactedFilters .sub-filter-pill').forEach(function(b){
    var t = b.textContent.trim();
    b.classList.toggle('active',
      (val==='all' && t==='All') ||
      (val==='yes' && t.indexOf('Done')>=0) ||
...
window.aiSignupAdminToggleMsg = function(id){
  _aiSignupExpanded[id] = !_aiSignupExpanded[id];
  aiSignupAdminRenderList();
};

window.aiSignupAdminToggleContacted = function(id){
  var sub = (window._SUBMISSIONS||[]).find(function(s){ return s.id===id; });
  if(!sub) return;
  sub.contacted = !sub.contacted;
  var btn = document.getElementById('aiSignupConBtn_'+id);
  var row = document.getElementById('aiSignupCon_'+id);
  if(btn) btn.classList.toggle('on', sub.contacted);
  if(row){
    row.classList.toggle('contacted', sub.contacted);
    var span = row.querySelector('span');
    if(span) span.textContent = sub.contacted?'✓ Contacted Back':'Not Contacted Yet';
  }
  try { if(typeof updateSubmission === 'function') updateSubmission(sub).catch(function(){}); } catch(_){}
};

window.aiSignupAdminChangeStatus = function(id, val){
  var sub = (window._SUBMISSIONS||[]).find(function(s){ return s.id===id; });
  if(!sub) return;
  sub.status = val;
  var card = document.getElementById('aiSignupCard_'+id);
  if(card){
    card.className = 'sub-card status-'+val.toLowerCase();
    var badge = card.querySelector('.sub-status-badge');
    if(badge){ badge.className = 'sub-status-badge sub-status-'+val.toLowerCase(); badge.textContent = val; }
...
window.aiSignupDownloadPDF = function(){
  var rows = _aiSignupGetFiltered();
  if(!rows.length){ alert('No AI signups to export.'); return; }

  var tbody = rows.map(function(s, i){
    var moti = (s.services||[]).join(', ') || '—';
    // Extract website / tech / contact-time from the message blob we
    // formatted at submission time (see aiSignupFinalize).
    var msg = s.message || '';
    function pluck(prefix){
      var line = msg.split('\n').find(function(l){ return l.indexOf(prefix) === 0; });
      return line ? line.slice(prefix.length).trim() : '';
    }
    var hasWebsite   = pluck('Has a website? ');
    var websiteUrl   = pluck('Website URL: ');
    var wantsPricing = pluck('Interested in $100 basic site? ');
    var techstack    = pluck('Current tech stack: ');
    var contactTime  = pluck('Best time / method: ');

    var websiteCell = hasWebsite === 'Yes'
      ? '<strong>Yes</strong>' + (websiteUrl ? '<br><span style="font-size:10px;color:#666;word-break:break-all">'+_escHTML(websiteUrl)+'</span>' : '')
      : hasWebsite === 'No'
        ? '<strong>No</strong>' + (wantsPricing ? '<br><span style="font-size:10px;color:#666">Pricing: '+_escHTML(wantsPricing)+'</span>' : '')
        : _escHTML(hasWebsite || '—');
    ...
    return '<tr style="background:'+(i%2===0?'#f9f9f9':'#fff')+'">'
      ...
      + '<td style="padding:10px 12px;border-bottom:1px solid #e5e5e5">'
        + _escHTML(s.name||'—')
        + '<br><span style="font-size:11px;color:#666">'+_escHTML(s.email||'—')+'</span>'
        + (s.phone ? '<br><span style="font-size:11px;color:#666">'+_escHTML(s.phone)+'</span>' : '')
      + '</td>'
      + '<td style="padding:10px 12px;border-bottom:1px solid #e5e5e5;font-size:11px;max-width:140px">'+websiteCell+'</td>'
      + '<td style="padding:10px 12px;border-bottom:1px solid #e5e5e5;font-size:11px;max-width:180px">'+_escHTML(moti)+'</td>'
      + '<td style="padding:10px 12px;border-bottom:1px solid #e5e5e5;font-size:12px;max-width:160px">'+_escHTML(techstack||'—')+'</td>'
      + '<td style="padding:10px 12px;border-bottom:1px solid #e5e5e5;font-size:11px;max-width:180px;white-space:pre-wrap">'+_escHTML(contactTime||'—')+'</td>'
      + '<td style="padding:10px 12px;border-bottom:1px solid #e5e5e5;text-align:center"><span style="padding:3px 9px;border-radius:20px;font-size:10px;font-weight:700;background:'+(s.status==='New'?'#fff3cd':s.status==='Read'?'#cce5ff':'#d4edda')+';color:'+(s.status==='New'?'#856404':s.status==='Read'?'#004085':'#155724')+'">'+_escHTML(s.status||'New')+'</span></td>'
      + '<td style="padding:10px 12px;border-bottom:1px solid #e5e5e5;text-align:center">'+(s.contacted?'<span style="color:#28a745;font-weight:700">✓ Yes</span>':'<span style="color:#999">No</span>')+'</td>'
      + '<td style="padding:10px 12px;border-bottom:1px solid #e5e5e5;font-size:11px;color:#666">'+_escHTML(s.datetime||'')+'</td>'
      + '</tr>';
  }).join('');
  ...
  var html = '<!DOCTYPE html><html><head><meta charset="utf-8">'
    + '<title>Mirror Advisors — AI Signups</title>'
    + '<style>body{font-family:Helvetica,Arial,sans-serif;font-size:13px;color:#1a1a1a;margin:0;padding:0}'
    + '.header{background:#080B1A;color:#ECA934;padding:28px 36px;display:flex;align-items:center;justify-content:space-between}'
    + '.header h1{margin:0;font-size:20px;font-weight:900;letter-spacing:-.02em}'
    + '.header p{margin:4px 0 0;font-size:12px;opacity:.6;color:#fff}'
    ...
    + '@media print{body{-webkit-print-color-adjust:exact}}'
    + '</style></head><body>'
    + '<div class="header"><div><h1>Mirror Advisors</h1><p>AI Free-Chatbot-Session Sign-ups</p></div>'
    + '<div style="text-align:right;color:#fff;font-size:12px;opacity:.6">'+new Date().toLocaleDateString('en-US',{year:'numeric',month:'long',day:'numeric'})+'</div></div>'
    + '<div class="body">'
    + '<div class="meta">'+rows.length+' signup'+(rows.length===1?'':'s')+' exported</div>'
    + '<table><thead><tr><th>#</th><th>Contact</th><th>Website</th><th>Motivation</th><th>Tech Stack</th><th>Best Time / Method</th><th>Status</th><th>Contacted</th><th>Date</th></tr></thead><tbody>'
    + tbody
    + '</tbody></table></div></body></html>';
  ...
  var a    = document.createElement('a');
  a.href   = url;
  a.download = 'mirror-advisors-ai-signups-' + new Date().toISOString().split('T')[0] + '.html';
  a.click();
  URL.revokeObjectURL(url);
  if(window._adminToast) window._adminToast('Export downloaded — open in browser and print to PDF ✓');
};
...
var _faqs=[
  {q:"Do I really have to start with Scope?",a:"Yes, for every new client. It is the two- to four-week audit and blueprint covered in <em>Step One &mdash; Always</em> above. Without that document we would be building on assumptions."},
  {q:"What if I only need one practice &mdash; Zoho, marketing, or AI?",a:"That is exactly how most engagements start. The three practices in <em>Core Services</em> above run independently: pick one or bundle them. Scope confirms which actually applies to your situation."},
  {q:"How do I choose between the four engagement models?",a:"Scope produces the recommendation. In short: pick <em>ERP Projects</em> for a fixed-scope build with a hard go-live, <em>Infinity Mirror</em> for continuous evolution, <em>Bank of Hours</em> for flexible short-cycle work, and <em>Support Only</em> for ongoing SLA on a live system."},
  {q:"Can I switch engagement models later?",a:"Yes, and most clients do. A typical arc is ERP Project or Infinity through go-live, then Support Only or Bank of Hours for the long tail. The Full Journey above shows the shape."},
  {q:"How long does Scope take and what does it cost?",a:"Two to four weeks, priced as a fixed fee based on company size and system count. You know the number before we start."},
  {q:"What if Scope shows we are not the right fit?",a:"We tell you, honestly, in the readout. We would rather lose the project than deliver the wrong solution and burn the relationship."},
];
...
// ── PH TEAM ─────────────────────────────────────────────────────────────────
// Unlisted operations-team page at /ph-team. When it mounts, populate the
// .pft-social container with the currently-enabled social icons — same
// renderer the footer uses so both stay in sync. pathToInitKey converts the
// dashed URL slug to an underscore, so the key here is ph_team.
_INIT.ph_team=function(){
  if(typeof _renderSocialIcons==='function') _renderSocialIcons();
};

// ── /artificial-intelligence · conversational sign-up form ─────────────
// A step-through form styled as a chat. Each step renders one bot bubble
// + one input row, the visitor answers, we collapse the row into a user
// bubble, and reveal the next step after a small delay so the exchange
// reads as a conversation. Config lives in AI_SIGNUP_STEPS below —
// text / tel / email / textarea / choice / multiselect types supported.
_INIT.artificial_intelligence=function(){
  if(window._aiSignupBound) return;
  window._aiSignupBound = true;
  window._aiSignup = { step: 0, answers: {} };
};

var AI_SIGNUP_STEPS = [
  { key:'name',        type:'text',        prompt:"First things first — what's your full name?",                     placeholder:"First and last" },
  { key:'phone',       type:'tel',         prompt:function(a){ var n=(a.name||'').split(/\s+/)[0]; return (n?"Nice to meet you, "+n+". ":"")+"What's the best number to reach you on?"; }, placeholder:"(xxx) xxx-xxxx" },
  { key:'email',       type:'email',       prompt:"And your business email?",                                        placeholder:"you@yourcompany.com" },
  { key:'hasWebsite',  type:'choice',      prompt:"Do you have a business website?",                                 options:['Yes','No'] },
  { key:'websiteUrl',  type:'text',        prompt:"Great — what's the URL?",                                         placeholder:"yourcompany.com",
    condition:function(a){ return a.hasWebsite === 'Yes'; } },
  { key:'wantsWebsite',type:'choice',      prompt:"No problem. Would you like us to send pricing for a basic business website? (starts at $100)", options:['Yes, send pricing','No thanks'],
    condition:function(a){ return a.hasWebsite === 'No'; } },
  { key:'motivation',  type:'multiselect', prompt:"What's your primary motivation for adding AI? Pick any that apply.",
    options:['Save money','Improve processes','Replace outdated software','Get ahead of competition'] },
  { key:'techstack',   type:'text',        prompt:"What tools are you running today? (e.g. Microsoft, Salesforce, QuickBooks)",
    placeholder:"Your current stack" },
  { key:'contactTime', type:'textarea',    prompt:"Last one — when and how should we reach you for the session?",
    placeholder:"e.g. Weekdays after 2pm CST, email preferred" },
];

function _aiSignupEsc(s){
  return String(s == null ? '' : s)
    .replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;')
    .replace(/"/g,'&quot;').replace(/'/g,'&#39;');
}

window.aiSignupStart = function(){
  var intro = document.getElementById('aiSignupIntro');
  var chat  = document.getElementById('aiSignupChat');
  if(intro) intro.style.display = 'none';
  if(chat)  chat.style.display  = 'flex';
  if(!window._aiSignup) window._aiSignup = { step: 0, answers: {} };
  window._aiSignup.step = 0;
  window._aiSignup.answers = {};
  aiSignupRenderNext();
};

window.aiSignupRenderNext = function(){
  var state = window._aiSignup;
  var stream = document.getElementById('aiSignupStream');
  if(!stream) return;

  // Skip conditional steps that don't apply given the current answers
  while(state.step < AI_SIGNUP_STEPS.length){
    var s = AI_SIGNUP_STEPS[state.step];
    if(s.condition && !s.condition(state.answers)){ state.step++; continue; }
    break;
  }

  if(state.step >= AI_SIGNUP_STEPS.length){
    aiSignupFinalize();
    return;
  }

  var step = AI_SIGNUP_STEPS[state.step];
  var prompt = typeof step.prompt === 'function' ? step.prompt(state.answers) : step.prompt;

  // Bot message
  var msg = document.createElement('div');
  msg.className = 'ai-signup-bot ai-signup-msg-appear';
  msg.textContent = prompt;
  stream.appendChild(msg);

  // Input row
  var row = document.createElement('div');
  row.className = 'ai-signup-input-row ai-signup-msg-appear';
  row.style.animationDelay = '.18s';

  if(step.type === 'text' || step.type === 'tel' || step.type === 'email'){
    row.innerHTML =
      '<input type="'+step.type+'" class="ai-signup-input" id="aiSignupField" ' +
      'placeholder="'+_aiSignupEsc(step.placeholder||'')+'" ' +
      'onkeydown="if(event.key===\'Enter\'){event.preventDefault();aiSignupSubmit(\''+step.key+'\')}" />' +
      '<button class="ai-signup-send" onclick="aiSignupSubmit(\''+step.key+'\')">' +
        'Send <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M5 12h14M12 5l7 7-7 7"/></svg>' +
      '</button>';
  } else if(step.type === 'textarea'){
    row.innerHTML =
      '<textarea class="ai-signup-input ai-signup-textarea" id="aiSignupField" ' +
      'placeholder="'+_aiSignupEsc(step.placeholder||'')+'"></textarea>' +
      '<button class="ai-signup-send" onclick="aiSignupSubmit(\''+step.key+'\')">' +
        'Send <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M5 12h14M12 5l7 7-7 7"/></svg>' +
      '</button>';
  } else if(step.type === 'choice'){
    var choicesHtml = step.options.map(function(opt){
      return '<button class="ai-signup-choice" onclick="aiSignupChoose(\''+step.key+'\',\''+_aiSignupEsc(opt)+'\')">'+_aiSignupEsc(opt)+'</button>';
    }).join('');
    row.classList.add('ai-signup-input-row-choices');
    row.innerHTML = choicesHtml;
  } else if(step.type === 'multiselect'){
    var checksHtml = step.options.map(function(opt){
      return '<label class="ai-signup-check"><input type="checkbox" data-multi-value="'+_aiSignupEsc(opt)+'" /><span>'+_aiSignupEsc(opt)+'</span></label>';
    }).join('');
    row.classList.add('ai-signup-input-row-multi');
    row.innerHTML =
      '<div class="ai-signup-multi-list">'+checksHtml+'</div>' +
      '<button class="ai-signup-send" onclick="aiSignupSubmitMulti(\''+step.key+'\')">' +
        'Send <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M5 12h14M12 5l7 7-7 7"/></svg>' +
      '</button>';
  }
  ...
  // Focus the input (not choices)
  setTimeout(function(){
    var f = document.getElementById('aiSignupField');
    if(f && typeof f.focus === 'function') try{ f.focus(); }catch(_){}
    stream.scrollTop = stream.scrollHeight;
  }, 260);
};

window.aiSignupSubmit = function(key){
  var f = document.getElementById('aiSignupField');
  var v = f ? String(f.value || '').trim() : '';
  if(!v) return;
  if(key === 'email' && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(v)){
    _aiSignupFieldError('That doesn’t look like a valid email — try again?');
    return;
  }
  _aiSignupRecord(key, v);
};

function _aiSignupFieldError(msg){
  var stream = document.getElementById('aiSignupStream');
  if(!stream) return;
  var existing = stream.querySelector('.ai-signup-field-error');
  if(existing) existing.parentNode.removeChild(existing);
  var err = document.createElement('div');
  err.className = 'ai-signup-field-error';
  err.style.cssText = 'font-family:Montserrat,sans-serif;font-size:12px;font-weight:600;color:#B54747;padding:6px 4px 0;';
  err.textContent = msg;
  var rows = stream.querySelectorAll('.ai-signup-input-row');
  var last = rows[rows.length - 1];
  if(last && last.parentNode){
    last.parentNode.insertBefore(err, last.nextSibling);
  } else {
    stream.appendChild(err);
  }
  var f = document.getElementById('aiSignupField');
  if(f && typeof f.focus === 'function') try{ f.focus(); f.select && f.select(); }catch(_){}
}

window.aiSignupChoose = function(key, value){
  _aiSignupRecord(key, value);
};

window.aiSignupSubmitMulti = function(key){
  var checks = document.querySelectorAll('.ai-signup-multi-list input[data-multi-value]:checked');
  if(!checks || !checks.length) return;
  var vals = Array.prototype.slice.call(checks).map(function(c){ return c.getAttribute('data-multi-value'); });
  _aiSignupRecord(key, vals);
};

function _aiSignupRecord(key, value){
  window._aiSignup.answers[key] = value;

  // Collapse the last input row into a user bubble
  var stream = document.getElementById('aiSignupStream');
  var rows = stream.querySelectorAll('.ai-signup-input-row');
  var last = rows[rows.length - 1];
  if(last){
    ...
    bubble.className = 'ai-signup-user ai-signup-msg-appear';
    bubble.textContent = display;
    stream.replaceChild(bubble, last);
  }

  // Advance and render next (with a brief delay for typing feel)
  window._aiSignup.step++;
  var typing = document.createElement('div');
  typing.className = 'ai-signup-typing';
...
function aiSignupFinalize(){
  ...
  setTimeout(function(){
    ...
    done.className = 'ai-signup-bot ai-signup-msg-final ai-signup-msg-appear';
    var name = (window._aiSignup.answers.name || '').split(/\s+/)[0] || 'there';
    done.innerHTML = '<div style="font-family:Montserrat,sans-serif;font-weight:800;font-size:11px;color:#ECA934;letter-spacing:.09em;text-transform:uppercase;margin-bottom:8px">Session queued</div>' +
                     'Got it, '+_aiSignupEsc(name)+' &mdash; we&#39;ll reach out within one business day to schedule your free chatbot session.';
    stream.appendChild(done);
    stream.scrollTop = stream.scrollHeight;

    // Write to the shared `submissions` table via Supabase — the existing
    // Postgres → Resend trigger on that table fires the notification email,
    // and the lead also shows up in the admin Submissions dashboard.
    try {
      var a = window._aiSignup.answers || {};
      var lines = ['— AI free-chatbot-session request —', ''];
      lines.push('Has a website? ' + (a.hasWebsite || '—'));
      if (a.websiteUrl)   lines.push('Website URL: '                    + a.websiteUrl);
      if (a.wantsWebsite) lines.push('Interested in $100 basic site? '  + a.wantsWebsite);
      lines.push(
        'Current tech stack: ' + (a.techstack   || '—'),
        'Best time / method: ' + (a.contactTime || '—')
      );
      ...
      var submission = {
        ...
        size:      '',
        timeline:  '',
        services:  Array.isArray(a.motivation) ? a.motivation : [],
        message:   lines.join('\n'),
        source:    'ai-signup',
        status:    'New',
        contacted: false,
      ...
      window._SUBMISSIONS = window._SUBMISSIONS || [];
      window._SUBMISSIONS.unshift(submission);
      console.log('[ai-signup] inserting into submissions:', submission);
      insertSubmission(submission)
        .then(function(ok){ console.log('[ai-signup] insert result:', ok); })
        .catch(function(e){ console.error('[ai-signup] insert error:', e); });
    } catch(e){ console.error('[ai-signup] finalize threw:', e); }
  }, 900);
}

// Copy an unlisted-page URL to the clipboard from the admin dashboard's
// "Unlisted Pages" card. Uses the async clipboard API where available,
// falls back to the old textarea trick for older browsers.
window.adminCopyUnlistedUrl=function(url,label){
  var doToast=function(msg){ if(window._adminToast) window._adminToast(msg); };
  if(navigator.clipboard && navigator.clipboard.writeText){
    navigator.clipboard.writeText(url).then(function(){
      doToast((label||'Link')+' copied ✓');
    }).catch(function(){ doToast('Copy failed — select and copy manually'); });
  } else {
    try {
...
// ── NAV PAGES CONFIG ─────────────────────────────────────────────────────────
// Entries with a `parent` field are sub-pages surfaced under the parent's
// hover-dropdown in the top nav (currently only 'services' has children).
// They render in the admin nav dashboard as regular rows so each can be
// toggled visible/hidden independently.
window._NAV_PAGES = [
  { key:'services',             label:'Services',                  enabled:true },
  { key:'systems-integration',  label:'Systems Integration',       enabled:true, parent:'services' },
  { key:'data-migration',       label:'Data Migration',            enabled:true, parent:'services' },
...
window._CHAT_TREE = {
  ...
  fallback: "The best next step is a quick call. No commitment, just a conversation.",
  topics: [
    {
      id: "services",
      label: "What we do",
      answer: "We do six things, all starting with a Mirror Scope. Which would you like to know about?",
      choices: [
        { label: "What is a Mirror Scope?", nodeId: "scope" },
        { label: "Systems Integration", nodeId: "systems-integration" },
        { label: "Data Migration", nodeId: "data-migration" },
        { label: "ERP Implementation", nodeId: "erp-implementation" },
        { label: "Custom Development", nodeId: "custom-development" },
        { label: "AI & Automation", nodeId: "ai-automation" },
        { label: "Zoho Consulting & Support", nodeId: "zoho" }
      ]
    },
    {
      id: "howwework",
      label: "How we work",
      answer: "Every project starts with a Mirror Scope. What would you like to know?",
      choices: [
        { label: "What is a Mirror Scope?", nodeId: "scope" },
        { label: "Who does the work?", nodeId: "hw_team" },
        { label: "How a project runs", nodeId: "hw_stages" },
        { label: "Who you work with", nodeId: "hw_fit" }
      ]
    },
    {
      id: "about",
      label: "About the firm",
      answer: "Mirror Advisors is a US-led systems integrator based in The Woodlands, Texas. What would you like to know?",
      choices: [
        { label: "Who founded the firm?", nodeId: "about_founder" },
        { label: "Where the name comes from", nodeId: "about_name" },
        { label: "Credentials", nodeId: "about_credentials" }
      ]
    },
    {
      id: "next",
      label: "Talk to us",
      answer: "The best next step is a conversation. We reply quickly.",
      choices: [
        { label: "Book a call", nodeId: "cta_book" }
      ]
    }
  ],
  nodes: {
    scope: { answer: "The Mirror Scope is a paid discovery phase. We map how your processes actually run today, document what the new system has to do, and produce an implementation plan with timelines and costs attached. That document is yours whether you build with us or not.", choices: [] },
    "systems-integration": { answer: "We connect the tools you already own so information moves between them automatically, in the right direction, without anyone copying and pasting.", choices: [] },
    "data-migration": { answer: "We move your history into a new system cleanly, mapped, validated before go-live and reconciled after, so you can prove the numbers tie out.", choices: [] },
    "erp-implementation": { answer: "We deploy the system your operation runs on, built around how your business actually runs. Selection through go-live and beyond.", choices: [] },
    "custom-development": { answer: "When off-the-shelf software doesn't do it, we build it. Custom modules, portals, integrations, workflow logic and standalone applications.", choices: [] },
    "ai-automation": { answer: "AI applied where it earns its place, inside systems that are already connected properly. We built our own AI platform to run this company, and we use it every day.", choices: [] },
    zoho: { answer: "Zoho remains the platform we've implemented more than any other. Implementation, migration, custom development, integration, and ongoing support. Zoho Authorized Partner.", choices: [] },
    hw_team: { answer: "Sales, consulting and the Mirror Scope are handled by our US team. Paul Trinidad, our founder, is involved in every project from the first call through go-live. Project management and development run across our teams in the Philippines and India.", choices: [] },
    hw_stages: { answer: "Mirror Scope, then approval, then build in stages with a weekly project meeting, then training on your data, then go-live with us alongside you and support afterward.", choices: [] },
    hw_fit: { answer: "Our best work is with established teams that have outgrown their current setup and have leadership ready to commit to fixing it. If that isn't you yet, tell us anyway. We'd rather point you somewhere useful than sell you something that won't work.", choices: [] },
    about_founder: { answer: "Mirror Advisors was founded in December 2023 by Paul Trinidad, who spent three years at Zoho before founding the firm.", choices: [] },
    about_name: { answer: "A mirror is for looking closely at something you thought you already knew. That is what the first phase of every project is: a clear look at how your business actually runs.", choices: [] },
    about_credentials: { answer: "Zoho Authorized Partner. Our team holds certifications across Zoho Creator, Zoho Workplace and Zoho CRM.", choices: [] },
    cta_book: { answer: "Head to the Contact page and tell us what's broken. We reply quickly.", choices: [] }
  }
};
...
window.adminGoHome = function(){
  ...
  ['adminDashContent','adminSocialContent',
   'adminNavContent','adminSubContent','adminAiSignupsContent'].forEach(function(id){
    var el = document.getElementById(id);
  ...
  ['tabDashboard','tabSocial','tabNav','tabSub','tabAiSignups','tabSettings'].forEach(function(id){
    var el = document.getElementById(id);
...
window.adminRenderHome = function(container){
  ...
  wrap.innerHTML =
    ...
    + _adminHomeCard('ai-signups',  '🤖', 'rgba(236,169,52,.15)',  'AI Signups',       'View free-chatbot-session requests · export as PDF')
    + '</div>'
    // Unlisted pages — pages that exist on the site but aren't linked
    // in any nav, footer, or sitemap. Copy the URL to share it directly.
    + '<div style="margin-top:24px;">'
      + '<div style="font-family:Montserrat,sans-serif;font-size:10px;font-weight:800;text-transform:uppercase;letter-spacing:.1em;color:var(--dim);margin-bottom:14px;">Unlisted Pages</div>'
      + '<div style="background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.08);border-radius:12px;padding:14px 16px;display:flex;align-items:center;gap:14px;">'
        + '<div style="width:38px;height:38px;border-radius:10px;background:rgba(236,169,52,.1);border:1px solid rgba(236,169,52,.28);display:flex;align-items:center;justify-content:center;flex-shrink:0;font-size:17px;line-height:1">🔒</div>'
        + '<div style="flex:1;min-width:0;">'
          + '<div style="font-family:Montserrat,sans-serif;font-size:13px;font-weight:800;color:var(--tx);margin-bottom:3px;">PH Team</div>'
          + '<div style="font-family:\'SF Mono\',ui-monospace,monospace;font-size:11.5px;color:var(--dim);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;" title="https://mirroradvisors.com/ph-team">https://mirroradvisors.com/ph-team</div>'
        + '</div>'
        + '<button onclick="adminCopyUnlistedUrl(\'https://mirroradvisors.com/ph-team\',\'PH Team URL\')" style="background:rgba(236,169,52,.12);border:1px solid rgba(236,169,52,.3);color:#ECA934;font-family:Montserrat,sans-serif;font-size:11px;font-weight:800;letter-spacing:.06em;padding:8px 14px;border-radius:8px;cursor:pointer;text-transform:uppercase;flex-shrink:0;transition:background .18s ease;" onmouseover="this.style.background=\'rgba(236,169,52,.22)\'" onmouseout="this.style.background=\'rgba(236,169,52,.12)\'">Copy</button>'
      + '</div>'
    + '</div>'
    + (isAdmin
      ? '<div style="margin-top:24px;">'
...
window.adminSelectDash = function(mode){
  ...
  var badges = { cases:'Case Studies', chatbot:'Chatbot', social:'Social Media',
                 navigation:'Navigation', submissions:'Submissions', 'ai-signups':'AI Signups' };
...
window.adminGoSettings = function(){
  ...
  ['tabDashboard','tabSocial','tabNav','tabSub','tabAiSignups'].forEach(function(id){
    var el = document.getElementById(id);
...
window.adminSwitchMode = function(){
  ...
  var modes = ['cases','chatbot','social','navigation','submissions','ai-signups'];
  modes.forEach(function(m){
    var pickerKey = m === 'ai-signups' ? 'AiSignups' : (m.charAt(0).toUpperCase() + m.slice(1));
    var id = 'picker' + pickerKey;
...
window.adminSetTab = function(tab){
  ...
  ['adminDashContent', 'adminSocialContent', 'adminNavContent', 'adminSubContent', 'adminAiSignupsContent', 'adminSettingsContent', 'adminHomeContent'].forEach(function(id){
    var el = document.getElementById(id);
  ...
  } else if(tab === 'ai-signups'){
    var t = document.getElementById('tabAiSignups');
    var p = document.getElementById('adminAiSignupsContent');
    if(t){ t.style.display='flex'; t.classList.add('active'); }
    if(p){
      p.style.display='flex';
      p.innerHTML='';
      p._supaFetched = false;
      adminRenderAiSignupsDash(p);
    }
...
var _CT_GOTO_PAGES = [
  ...
  { key:'services/systems-integration',     label:'Systems Integration' },
  { key:'services/data-migration',          label:'Data Migration' },
  { key:'services/erp-implementation',      label:'ERP Implementation' },
  { key:'services/custom-development',      label:'Custom Development' },
  { key:'services/ai-automation',           label:'AI & Automation' },
  { key:'services/zoho',                    label:'Zoho Consulting & Support' },
...
// ─────────────────────────────────────────────────────────────────
// POC LAB — experimental interactive section on
// /services/custom-ai-application. Handlers are attached to `window`
// at module load so the inline HTML onclick / onchange attributes
// (which render via dangerouslySetInnerHTML and therefore CAN'T
// import from an ES module) find them by name. Everything is
// client-side; the only server round-trip is POST /api/poc-notify.
// ─────────────────────────────────────────────────────────────────
if (typeof window !== 'undefined') {

  // 1MB raw-file cap. After FileReader.readAsDataURL the base64 grows
  // by ~4/3, so 1 MB in becomes ~1.33 MB in the JSON payload — within
  // the 2 MB API body-parser limit on /api/poc-notify.
  var _POC_LOGO_MAX_BYTES = 1024 * 1024;

  // In-memory only. Never touched by localStorage / cookies / Supabase.
  window._pocLogoData = null;

  // Escape user-supplied strings before injecting into the preview HTML.
  function _pocEsc(s) {
    return String(s == null ? '' : s)
      .replace(/&/g, '&amp;')
      .replace(/</g, '&lt;')
  ...
  // Any click inside a rendered PoC mockup — nav item, CTA, stat card,
  // activity row, the floating chat bubble — routes the visitor to
  // Mirror Advisors' contact page. The mockup pretends to be another
  // company's site so every "click" is a demonstration + a friendly
  // funnel toward booking a call with us.
  window.pocGoContact = function () {
    if (typeof window.go === 'function') {
      try { window.go('contact'); return; } catch (_) {}
    }
  ...
  // File picker → data URL. Runs on the file input's onchange.
  window.pocLogoUpload = function (input) {
    ...
    if (file.size > _POC_LOGO_MAX_BYTES) {
      alert('Logo must be under 1 MB. Try a smaller or compressed version.');
      input.value = '';
      return;
    }
    var reader = new FileReader();
    reader.onload = function (e) {
      window._pocLogoData = e.target.result || null;
      var preview = document.getElementById('pocLogoPreview');
      if (preview && window._pocLogoData) {
        preview.src = window._pocLogoData;
  ...
  // Reset the form + preview back to the initial state.
  window.pocReset = function () {
    var form = document.getElementById('pocFormElement');
    if (form && typeof form.reset === 'function') form.reset();
    var preview = document.getElementById('pocLogoPreview');
    if (preview) { preview.src = ''; preview.classList.remove('has-logo'); }
    window._pocLogoData = null;
    var out = document.getElementById('pocPreview');
    if (out) {
      out.classList.remove('filled');
      out.innerHTML =
        '<div class="poc-preview-empty">' +
          '<div class="poc-preview-empty-icon">&#9654;</div>' +
          '<div>Fill in the form and click <strong>Generate PoC</strong> to see a live preview.</div>' +
        '</div>';
    }
  };

  // Validate + render the mockup + fire-and-forget the email notification.
  window.pocGenerate = function () {
    var nameEl    = document.getElementById('pocCompanyName');
    var detailsEl = document.getElementById('pocDetails');
    var typeRadio = document.querySelector('input[name="pocType"]:checked');

    var name    = (nameEl && nameEl.value    || '').trim();
    var details = (detailsEl && detailsEl.value || '').trim();
    var pocType = typeRadio ? typeRadio.value : '';
    var logo    = window._pocLogoData || null;

    var errors = [];
    if (!name)                                    errors.push('Company name is required.');
    if (!details)                                 errors.push('Please add a short description of your business.');
    if (!pocType)                                 errors.push('Pick a PoC type (Website or Custom CRM).');
    if (errors.length) { alert(errors.join('\n')); return; }

    // Render the preview immediately — pure client-side, no server
    // round-trip. Email notification was removed at the user's
    // request; nothing about the PoC leaves the browser.
    var out = document.getElementById('pocPreview');
    if (out) {
      var mockup = pocType === 'Website'
        ? _pocRenderWebsite(name, details, logo)
        : _pocRenderCrm(name, details, logo);
      out.classList.add('filled');
      out.innerHTML = mockup;
    }
  };

  // ─── mockup templates ─────────────────────────────────────────

  // A tiny mock landing page in a browser chrome.

  // Switch between views within a rendered PoC mockup. Called by the
  // nav items — swaps display:block/none on all .poc-view children
  // that share the same [data-mockup] and toggles [data-active] on
  // the matching nav item so it lights up in forest ink.
  window.pocSwitchView = function (mockupId, viewName) {
    var scope = document.querySelector('[data-mockup="' + mockupId + '"]');
    if (!scope) return;
    var views = scope.querySelectorAll('.poc-view');
    for (var i = 0; i < views.length; i++) {
      views[i].style.display = views[i].getAttribute('data-view') === viewName ? '' : 'none';
    }
    var navs = scope.querySelectorAll('.poc-nav-item');
    for (var j = 0; j < navs.length; j++) {
      var isActive = navs[j].getAttribute('data-view') === viewName;
      navs[j].setAttribute('data-active', isActive ? 'true' : 'false');
    }
  };

  // Website mockup — Ease Health style, now a proper multi-view site.
  // Nav items switch between Home / Pricing / About views. Only the
  // CTA pills ("Book a Demo", "Get started", "Talk to sales") route
  // to Mirror Advisors' /contact page — nav browsing stays inside
  // the mockup so it feels like a real website.
  function _pocRenderWebsite(name, details, logoDataUrl) {
    var safeName    = _pocEsc(name);
    var safeDetails = _pocEsc(details).slice(0, 240);
    var domain      = safeName.toLowerCase().replace(/[^a-z0-9]+/g, '').slice(0, 22) || 'yourcompany';
    var initials    = safeName.replace(/[^A-Za-z ]/g, '').split(/\s+/).map(function (w) { return w[0] || ''; }).join('').slice(0, 2).toUpperCase() || safeName.slice(0, 2).toUpperCase();
    var logoBlock   = logoDataUrl
      ? '<img src="' + logoDataUrl + '" alt="' + safeName + ' logo" style="max-height:26px;max-width:110px;object-fit:contain">'
      : '<div style="width:28px;height:28px;border-radius:7px;background:#0f3e17;color:#fffefc;display:inline-flex;align-items:center;justify-content:center;font-family:Georgia,serif;font-size:13px;font-weight:400;font-style:italic;letter-spacing:-.02em">' + initials + '</div>';
    ...
    return [
      // Outermost wrapper — cream page canvas, data-mockup ID for view switching
      '<div data-mockup="'+MID+'" style="background:#fffefc;border-radius:12px;overflow:hidden;display:flex;flex-direction:column;flex:1;min-height:100%;position:relative">',

      // Browser chrome
      '<div style="display:flex;align-items:center;gap:6px;padding:10px 14px;background:#efeeeb;border-bottom:1px solid #e5e3df;flex-shrink:0">',
      ...
      '  <span style="margin-left:8px;font-size:10px;font-family:'+SANS+';color:#666;background:#fffefc;padding:3px 10px;border-radius:6px">' + domain + '.com</span>',
      '  <span style="margin-left:auto;font-size:8px;padding:2px 7px;border-radius:999px;background:#0f3e17;color:#fffefc;font-weight:600;font-family:'+SANS+';letter-spacing:.08em">POC PREVIEW</span>',
      '</div>',

      // Nav row — logo + view-switching nav items + Book a Demo CTA
      '<div style="display:flex;align-items:center;justify-content:space-between;padding:18px 28px;background:#fffefc;border-bottom:1px solid #efeeeb;flex-shrink:0">',
      '  <div class="poc-interactive" onclick="pocSwitchView(\''+MID+'\',\'home\')" style="display:flex;align-items:center;gap:12px">',
      logoBlock,
      '    <div style="font-family:'+SERIF+';font-size:22px;font-weight:300;color:#0f3e17;letter-spacing:-.01em;line-height:1">' + safeName + '</div>',
      '  </div>',
      '  <div style="display:flex;gap:22px;font-family:'+SANS+';font-size:13px">',
      '    <span class="poc-nav-item" data-view="home"    data-active="true"  onclick="pocSwitchView(\''+MID+'\',\'home\')">Home</span>',
      '    <span class="poc-nav-item" data-view="pricing" data-active="false" onclick="pocSwitchView(\''+MID+'\',\'pricing\')">Pricing</span>',
      '    <span class="poc-nav-item" data-view="about"   data-active="false" onclick="pocSwitchView(\''+MID+'\',\'about\')">About</span>',
      '  </div>',
      '  <div class="poc-interactive" onclick="pocGoContact()" style="padding:8px 16px;background:#0f3e17;color:#fffefc;border-radius:14px;font-family:'+SANS+';font-size:12px;font-weight:400">Book a Demo &rarr;</div>',
      '</div>',

      // ─── HOME VIEW ────────────────────────────────────────────────
      '<div class="poc-view" data-view="home" style="flex:1;display:flex;flex-direction:column;background:#fffefc">',
      '  <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;padding:22px;flex:1">',
      // Left panel — Keylime Wash
      '    <div style="background:#e1f4df;border-radius:14px;padding:32px 30px;display:flex;flex-direction:column;justify-content:space-between">',
      '      <div>',
      '        <span style="display:inline-block;font-family:'+SANS+';font-size:11px;font-weight:600;letter-spacing:.09em;text-transform:uppercase;color:#0f3e17;margin-bottom:16px">NEW &middot; FOR CUSTOMERS</span>',
      '        <div style="font-family:'+SERIF+';font-size:32px;font-weight:300;line-height:1.1;color:#0f3e17;letter-spacing:-.02em;margin-bottom:14px">Built for<br>' + safeName + ' <em style="font-style:italic">customers.</em></div>',
      '        <div style="font-family:'+SANS+';font-size:13px;font-weight:400;line-height:1.55;color:#222">' + safeDetails + '</div>',
      '      </div>',
      '      <div style="display:flex;gap:10px;margin-top:20px">',
      '        <div class="poc-interactive" onclick="pocGoContact()" style="padding:12px 20px;background:#0f3e17;color:#fffefc;border-radius:14px;font-family:'+SANS+';font-size:13px;font-weight:400">Get started &rarr;</div>',
      '        <div class="poc-interactive" onclick="pocSwitchView(\''+MID+'\',\'pricing\')" style="padding:12px 18px;background:#fffefc;color:#0f3e17;border-radius:14px;font-family:'+SANS+';font-size:13px;font-weight:400">See pricing</div>',
      '      </div>',
      '    </div>',
      // Right panel — Slate Hush
      '    <div style="background:#b6ced5;border-radius:14px;padding:20px;display:flex;flex-direction:column;gap:10px">',
      '      <div style="background:#fffefc;border-radius:12px;padding:16px 18px">',
      '        <span style="display:inline-block;font-family:'+SANS+';font-size:10px;font-weight:600;letter-spacing:.09em;text-transform:uppercase;color:#0f3e17;margin-bottom:6px">Fast Setup</span>',
      '        <div style="font-family:'+SERIF+';font-size:20px;font-weight:300;color:#0f3e17;line-height:1.1;letter-spacing:-.01em;margin-bottom:4px">Live in under a week.</div>',
      '        <div style="font-family:'+SANS+';font-size:11.5px;color:#222;line-height:1.5">From scope to production, no waiting.</div>',
      '      </div>',
      '      <div style="background:#fffefc;border-radius:12px;padding:16px 18px">',
      '        <span style="display:inline-block;font-family:'+SANS+';font-size:10px;font-weight:600;letter-spacing:.09em;text-transform:uppercase;color:#0f3e17;margin-bottom:6px">Built to Adapt</span>',
      '        <div style="font-family:'+SERIF+';font-size:20px;font-weight:300;color:#0f3e17;line-height:1.1;letter-spacing:-.01em;margin-bottom:4px">Bends to your team.</div>',
      '        <div style="font-family:'+SANS+';font-size:11.5px;color:#222;line-height:1.5">Configure workflows without a developer in the room.</div>',
      '      </div>',
      '      <div style="background:#fffefc;border-radius:12px;padding:16px 18px">',
      '        <span style="display:inline-block;font-family:'+SANS+';font-size:10px;font-weight:600;letter-spacing:.09em;text-transform:uppercase;color:#0f3e17;margin-bottom:6px">You Own It</span>',
      '        <div style="font-family:'+SERIF+';font-size:20px;font-weight:300;color:#0f3e17;line-height:1.1;letter-spacing:-.01em;margin-bottom:4px">Your team, your stack.</div>',
      '        <div style="font-family:'+SANS+';font-size:11.5px;color:#222;line-height:1.5">Fits into your existing infrastructure.</div>',
      '      </div>',
      '    </div>',
      '  </div>',
      // Investor logos (Home only)
      '  <div style="display:flex;justify-content:space-around;align-items:center;padding:20px 28px;background:#fffefc;border-top:1px solid #efeeeb">',
      '    <div style="font-family:'+SERIF+';font-size:16px;font-weight:300;color:#222;font-style:italic;letter-spacing:-.01em">Sequoia</div>',
      '    <div style="font-family:'+SANS+';font-size:13px;font-weight:600;color:#222;letter-spacing:.06em">a16z</div>',
      '    <div style="font-family:'+SERIF+';font-size:16px;font-weight:300;color:#222;letter-spacing:-.01em">Accel</div>',
      '    <div style="font-family:'+SANS+';font-size:11px;font-weight:400;color:#222;letter-spacing:.16em">GENERAL CATALYST</div>',
      '  </div>',
      '</div>',

      // ─── PRICING VIEW ─────────────────────────────────────────────
      '<div class="poc-view" data-view="pricing" style="display:none;flex:1;padding:36px 32px;background:#fffefc;overflow:auto">',
      '  <div style="text-align:center;margin-bottom:32px">',
      '    <span style="display:inline-block;font-family:'+SANS+';font-size:11px;font-weight:600;letter-spacing:.09em;text-transform:uppercase;color:#0f3e17;margin-bottom:12px">Simple, scales with you</span>',
      '    <div style="font-family:'+SERIF+';font-size:38px;font-weight:300;line-height:1.1;color:#0f3e17;letter-spacing:-.02em">Built for growth.</div>',
      '    <div style="font-family:'+SANS+';font-size:13px;color:#222;line-height:1.55;margin-top:8px;max-width:480px;margin-left:auto;margin-right:auto">Every plan includes full source-code handover and same-week onboarding.</div>',
      '  </div>',
      '  <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:16px;max-width:900px;margin:0 auto">',
      // Starter
      '    <div style="background:#e1f4df;border-radius:14px;padding:26px 22px;display:flex;flex-direction:column">',
      '      <span style="display:inline-block;font-family:'+SANS+';font-size:10px;font-weight:600;letter-spacing:.09em;text-transform:uppercase;color:#0f3e17;margin-bottom:10px">Starter</span>',
      '      <div style="font-family:'+SERIF+';font-size:36px;font-weight:300;color:#0f3e17;line-height:1;letter-spacing:-.02em">$49<span style="font-family:'+SANS+';font-size:13px;color:#222;font-weight:400">/mo</span></div>',
      '      <div style="font-family:'+SANS+';font-size:11px;color:#222;margin-top:6px;margin-bottom:16px">Perfect for early-stage teams.</div>',
      '      <div style="display:flex;flex-direction:column;gap:6px;flex:1;font-family:'+SANS+';font-size:12px;color:#222;line-height:1.55">',
      '        <div>&check; 3 seats</div>',
      '        <div>&check; 10K events / month</div>',
      '        <div>&check; Email support</div>',
      '      </div>',
      '      <div class="poc-interactive" onclick="pocGoContact()" style="padding:11px 18px;background:#fffefc;color:#0f3e17;border-radius:14px;font-family:'+SANS+';font-size:12px;font-weight:400;text-align:center;margin-top:18px">Get started</div>',
      '    </div>',
      // Growth (highlighted)
      '    <div style="background:#b1dbb8;border-radius:14px;padding:26px 22px;display:flex;flex-direction:column;position:relative">',
      '      <span style="position:absolute;top:-10px;right:16px;padding:4px 12px;border-radius:999px;background:#0f3e17;color:#fffefc;font-family:'+SANS+';font-size:9px;font-weight:600;letter-spacing:.08em">POPULAR</span>',
      '      <span style="display:inline-block;font-family:'+SANS+';font-size:10px;font-weight:600;letter-spacing:.09em;text-transform:uppercase;color:#0f3e17;margin-bottom:10px">Growth</span>',
      '      <div style="font-family:'+SERIF+';font-size:36px;font-weight:300;color:#0f3e17;line-height:1;letter-spacing:-.02em">$149<span style="font-family:'+SANS+';font-size:13px;color:#222;font-weight:400">/mo</span></div>',
      '      <div style="font-family:'+SANS+';font-size:11px;color:#222;margin-top:6px;margin-bottom:16px">Scale with confidence.</div>',
      '      <div style="display:flex;flex-direction:column;gap:6px;flex:1;font-family:'+SANS+';font-size:12px;color:#222;line-height:1.55">',
      '        <div>&check; 15 seats</div>',
      '        <div>&check; 100K events / month</div>',
      '        <div>&check; Priority support</div>',
      '        <div>&check; Custom integrations</div>',
      '      </div>',
      '      <div class="poc-interactive" onclick="pocGoContact()" style="padding:11px 18px;background:#0f3e17;color:#fffefc;border-radius:14px;font-family:'+SANS+';font-size:12px;font-weight:400;text-align:center;margin-top:18px">Book a Demo &rarr;</div>',
      '    </div>',
      // Scale
      '    <div style="background:#cfe7d3;border-radius:14px;padding:26px 22px;display:flex;flex-direction:column">',
      '      <span style="display:inline-block;font-family:'+SANS+';font-size:10px;font-weight:600;letter-spacing:.09em;text-transform:uppercase;color:#0f3e17;margin-bottom:10px">Scale</span>',
      '      <div style="font-family:'+SERIF+';font-size:36px;font-weight:300;color:#0f3e17;line-height:1;letter-spacing:-.02em">$499<span style="font-family:'+SANS+';font-size:13px;color:#222;font-weight:400">/mo</span></div>',
      '      <div style="font-family:'+SANS+';font-size:11px;color:#222;margin-top:6px;margin-bottom:16px">For high-volume teams.</div>',
      '      <div style="display:flex;flex-direction:column;gap:6px;flex:1;font-family:'+SANS+';font-size:12px;color:#222;line-height:1.55">',
      '        <div>&check; Unlimited seats</div>',
      '        <div>&check; Unlimited events</div>',
      '        <div>&check; Dedicated CSM</div>',
      '        <div>&check; SLA guarantees</div>',
      '      </div>',
      '      <div class="poc-interactive" onclick="pocGoContact()" style="padding:11px 18px;background:#fffefc;color:#0f3e17;border-radius:14px;font-family:'+SANS+';font-size:12px;font-weight:400;text-align:center;margin-top:18px">Talk to sales</div>',
      '    </div>',
      '  </div>',
      '  <div style="text-align:center;margin-top:32px;font-family:'+SANS+';font-size:12px;color:#666">Need custom terms? <span class="poc-interactive" onclick="pocGoContact()" style="color:#0f3e17;font-weight:500">Get in touch &rarr;</span></div>',
      '</div>',

      // ─── ABOUT VIEW ───────────────────────────────────────────────
      '<div class="poc-view" data-view="about" style="display:none;flex:1;padding:36px 32px;background:#fffefc;overflow:auto">',
      '  <div style="max-width:720px;margin:0 auto">',
      '    <span style="display:inline-block;font-family:'+SANS+';font-size:11px;font-weight:600;letter-spacing:.09em;text-transform:uppercase;color:#0f3e17;margin-bottom:12px">About us</span>',
      '    <div style="font-family:'+SERIF+';font-size:36px;font-weight:300;line-height:1.1;color:#0f3e17;letter-spacing:-.02em;margin-bottom:16px">A team building for<br><em style="font-style:italic">' + safeName + '\'s</em> future.</div>',
      '    <div style="font-family:'+SANS+';font-size:14px;color:#222;line-height:1.65;margin-bottom:32px">' + safeDetails + ' Our mission is to build the tools your team needs &mdash; not another SaaS you have to configure around your own workflow.</div>',
      '    <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:14px;margin-bottom:32px">',
      '      <div style="background:#e1f4df;border-radius:14px;padding:22px;text-align:center">',
      '        <div style="font-family:'+SERIF+';font-size:38px;font-weight:300;color:#0f3e17;line-height:1;letter-spacing:-.02em">2019</div>',
      '        <div style="font-family:'+SANS+';font-size:10px;color:#222;margin-top:6px;letter-spacing:.06em;text-transform:uppercase">Founded</div>',
      '      </div>',
      '      <div style="background:#b1dbb8;border-radius:14px;padding:22px;text-align:center">',
      '        <div style="font-family:'+SERIF+';font-size:38px;font-weight:300;color:#0f3e17;line-height:1;letter-spacing:-.02em">100+</div>',
      '        <div style="font-family:'+SANS+';font-size:10px;color:#222;margin-top:6px;letter-spacing:.06em;text-transform:uppercase">Teams shipped with</div>',
      '      </div>',
      '      <div style="background:#cfe7d3;border-radius:14px;padding:22px;text-align:center">',
      '        <div style="font-family:'+SERIF+';font-size:38px;font-weight:300;color:#0f3e17;line-height:1;letter-spacing:-.02em">14</div>',
      '        <div style="font-family:'+SANS+';font-size:10px;color:#222;margin-top:6px;letter-spacing:.06em;text-transform:uppercase">Countries</div>',
      '      </div>',
      '    </div>',
      '    <div style="background:#b6ced5;border-radius:14px;padding:28px;text-align:center">',
      '      <div style="font-family:'+SERIF+';font-size:22px;font-weight:300;color:#0f3e17;line-height:1.2;letter-spacing:-.01em;margin-bottom:10px">Want to see if we\'re the right fit?</div>',
      '      <div style="font-family:'+SANS+';font-size:12px;color:#222;line-height:1.55;margin-bottom:18px">Book a 30-minute intro call &mdash; no obligation, no sales pitch.</div>',
      '      <div class="poc-interactive" onclick="pocGoContact()" style="display:inline-block;padding:12px 24px;background:#0f3e17;color:#fffefc;border-radius:14px;font-family:'+SANS+';font-size:13px;font-weight:400">Book a Call &rarr;</div>',
      '    </div>',
      '  </div>',
      '</div>',

      // Floating chat bubble — always visible on any view
      '<div class="poc-chat-bubble" onclick="pocGoContact()"><span class="poc-chat-bubble-dot"></span>Book a Call with Mirror Advisors &rarr;</div>',

      '</div>',
    ].join('');
  }

  // CRM mockup — Ease Health style, now multi-view.
  // Sidebar switches between Dashboard / Customers / Analytics views.
  // Only action buttons ("Add Customer", "Export Report") route to
  // Mirror Advisors' /contact page.
  function _pocRenderCrm(name, details, logoDataUrl) {
    var safeName    = _pocEsc(name);
    var safeDetails = _pocEsc(details).slice(0, 200);
    var domain      = safeName.toLowerCase().replace(/[^a-z0-9]+/g, '').slice(0, 20) || 'yourcompany';
    var initials    = safeName.replace(/[^A-Za-z ]/g, '').split(/\s+/).map(function (w) { return w[0] || ''; }).join('').slice(0, 2).toUpperCase() || safeName.slice(0, 2).toUpperCase();
    var logoBlock   = logoDataUrl
      ? '<img src="' + logoDataUrl + '" alt="' + safeName + '" style="max-height:22px;max-width:80px;object-fit:contain;display:block">'
      : '<div style="width:28px;height:28px;border-radius:7px;background:#0f3e17;color:#fffefc;display:inline-flex;align-items:center;justify-content:center;font-family:Georgia,serif;font-size:13px;font-weight:400;font-style:italic;letter-spacing:-.02em">' + initials + '</div>';
    ...
    return [
      ...
      '  <span style="width:9px;height:9px;border-radius:50%;background:#ffbd2e"></span>',
      '  <span style="width:9px;height:9px;border-radius:50%;background:#28c840"></span>',
      '  <span style="margin-left:8px;font-size:10px;font-family:'+SANS+';color:#666;background:#fffefc;padding:3px 10px;border-radius:6px">app.' + domain + '.com/dashboard</span>',
      '  <span style="margin-left:auto;font-size:8px;padding:2px 7px;border-radius:999px;background:#0f3e17;color:#fffefc;font-weight:600;font-family:'+SANS+';letter-spacing:.08em">POC PREVIEW</span>',
      '</div>',

      '<div style="display:grid;grid-template-columns:180px 1fr;flex:1;min-height:0">',

      // Sidebar — nav items switch views
      '  <div style="background:#fffefc;padding:20px 14px;display:flex;flex-direction:column;border-right:1px solid #efeeeb">',
      '    <div class="poc-interactive" onclick="pocSwitchView(\''+MID+'\',\'dashboard\')" style="display:flex;align-items:center;gap:10px;padding-bottom:18px;border-bottom:1px solid #efeeeb;margin-bottom:16px">',
      logoBlock,
      '      <div style="font-family:'+SERIF+';font-size:18px;font-weight:300;color:#0f3e17;letter-spacing:-.01em;line-height:1;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">' + safeName + '</div>',
      '    </div>',
      '    <div style="display:flex;flex-direction:column;gap:2px;flex:1">',
      '      <div class="poc-nav-item poc-crm-nav" data-view="dashboard" data-active="true"  onclick="pocSwitchView(\''+MID+'\',\'dashboard\')" style="padding:9px 12px;border-radius:7px;font-family:'+SANS+';font-size:12px">Dashboard</div>',
      '      <div class="poc-nav-item poc-crm-nav" data-view="customers" data-active="false" onclick="pocSwitchView(\''+MID+'\',\'customers\')" style="padding:9px 12px;border-radius:7px;font-family:'+SANS+';font-size:12px">Customers</div>',
      '      <div class="poc-nav-item poc-crm-nav" data-view="analytics" data-active="false" onclick="pocSwitchView(\''+MID+'\',\'analytics\')" style="padding:9px 12px;border-radius:7px;font-family:'+SANS+';font-size:12px">Analytics</div>',
      '    </div>',
      '    <div style="padding-top:14px;border-top:1px solid #efeeeb;display:flex;align-items:center;gap:9px">',
      '      <div style="width:26px;height:26px;border-radius:50%;background:#0f3e17;color:#fffefc;display:flex;align-items:center;justify-content:center;font-family:'+SANS+';font-size:9px;font-weight:600;flex-shrink:0">AK</div>',
      '      <div style="min-width:0"><div style="font-family:'+SANS+';font-size:11.5px;font-weight:600;color:#0f3e17;line-height:1.1">Alex Kim</div><div style="font-family:'+SANS+';font-size:9.5px;color:#666;margin-top:1px">Admin</div></div>',
      '    </div>',
      '  </div>',

      // Main content pane — hosts all views
      '  <div style="background:#fffefc;overflow:auto">',

      // ─── DASHBOARD VIEW ────────────────────────────────────────────
      '    <div class="poc-view" data-view="dashboard" style="padding:28px 32px;display:flex;flex-direction:column;gap:20px">',
      '      <div>',
      '        <span style="display:inline-block;font-family:'+SANS+';font-size:11px;font-weight:600;letter-spacing:.09em;text-transform:uppercase;color:#0f3e17;margin-bottom:8px">DASHBOARD</span>',
      '        <div style="font-family:'+SERIF+';font-size:32px;font-weight:300;line-height:1.1;color:#0f3e17;letter-spacing:-.02em">Good afternoon, Alex.</div>',
      '        <div style="font-family:'+SANS+';font-size:13px;color:#222;line-height:1.55;margin-top:6px">' + safeName + ' &middot; ' + safeDetails + '</div>',
      '      </div>',
      '      <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:14px">',
      '        <div style="background:#e1f4df;border-radius:14px;padding:22px">',
      '          <span style="display:inline-block;font-family:'+SANS+';font-size:10px;font-weight:600;letter-spacing:.09em;text-transform:uppercase;color:#0f3e17;margin-bottom:10px">Active Customers</span>',
      '          <div style="font-family:'+SERIF+';font-size:36px;font-weight:300;line-height:1;color:#0f3e17;letter-spacing:-.02em">142</div>',
      '          <div style="font-family:'+SANS+';font-size:11px;color:#222;margin-top:8px">+8 this week</div>',
      '        </div>',
      '        <div style="background:#cfe7d3;border-radius:14px;padding:22px">',
      '          <span style="display:inline-block;font-family:'+SANS+';font-size:10px;font-weight:600;letter-spacing:.09em;text-transform:uppercase;color:#0f3e17;margin-bottom:10px">Open Tickets</span>',
      '          <div style="font-family:'+SERIF+';font-size:36px;font-weight:300;line-height:1;color:#0f3e17;letter-spacing:-.02em">23</div>',
      '          <div style="font-family:'+SANS+';font-size:11px;color:#222;margin-top:8px">2 SLA at risk</div>',
      '        </div>',
      '        <div style="background:#b1dbb8;border-radius:14px;padding:22px">',
      '          <span style="display:inline-block;font-family:'+SANS+';font-size:10px;font-weight:600;letter-spacing:.09em;text-transform:uppercase;color:#0f3e17;margin-bottom:10px">MRR</span>',
      '          <div style="font-family:'+SERIF+';font-size:30px;font-weight:300;line-height:1;color:#0f3e17;letter-spacing:-.02em">$142,400</div>',
      '          <div style="font-family:'+SANS+';font-size:11px;color:#222;margin-top:8px">+22% QoQ</div>',
      '        </div>',
      '      </div>',
      '      <div style="background:#b6ced5;border-radius:14px;padding:24px;flex:1;display:flex;flex-direction:column">',
      '        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px">',
      '          <div style="font-family:'+SERIF+';font-size:22px;font-weight:300;color:#0f3e17;letter-spacing:-.01em">Recent Activity</div>',
      '          <span class="poc-interactive" onclick="pocSwitchView(\''+MID+'\',\'customers\')" style="font-family:'+SANS+';font-size:10px;font-weight:600;letter-spacing:.09em;text-transform:uppercase;color:#0f3e17">VIEW ALL</span>',
      '        </div>',
      '        <div style="display:flex;flex-direction:column;gap:10px;flex:1">',
      '          <div style="background:#fffefc;border-radius:12px;padding:14px 16px;display:flex;align-items:center;gap:12px">',
      '            <div style="width:28px;height:28px;border-radius:50%;background:#0f3e17;color:#fffefc;display:flex;align-items:center;justify-content:center;font-family:'+SANS+';font-size:10px;font-weight:600;flex-shrink:0">JR</div>',
      '            <div style="flex:1;min-width:0"><div style="font-family:'+SANS+';font-size:12.5px;color:#222;line-height:1.4"><strong style="color:#0f3e17;font-weight:600">Jenna Robertson</strong> upgraded to Growth</div><div style="font-family:'+SANS+';font-size:10px;color:#666;margin-top:2px">Just now</div></div>',
      '            <span style="padding:5px 12px;border-radius:999px;background:#e1f4df;color:#0f3e17;font-family:'+SANS+';font-size:10px;font-weight:600;white-space:nowrap">New</span>',
      '          </div>',
      '          <div style="background:#fffefc;border-radius:12px;padding:14px 16px;display:flex;align-items:center;gap:12px">',
      '            <div style="width:28px;height:28px;border-radius:50%;background:#0f3e17;color:#fffefc;display:flex;align-items:center;justify-content:center;font-family:'+SANS+';font-size:10px;font-weight:600;flex-shrink:0">TF</div>',
      '            <div style="flex:1;min-width:0"><div style="font-family:'+SANS+';font-size:12.5px;color:#222;line-height:1.4"><strong style="color:#0f3e17;font-weight:600">Trevor Ferraro</strong> resolved 3 tickets</div><div style="font-family:'+SANS+';font-size:10px;color:#666;margin-top:2px">18 min ago</div></div>',
      '            <span style="padding:5px 12px;border-radius:999px;background:#cfe7d3;color:#0f3e17;font-family:'+SANS+';font-size:10px;font-weight:600;white-space:nowrap">Resolved</span>',
      '          </div>',
      '          <div style="background:#fffefc;border-radius:12px;padding:14px 16px;display:flex;align-items:center;gap:12px">',
      '            <div style="width:28px;height:28px;border-radius:50%;background:#0f3e17;color:#fffefc;display:flex;align-items:center;justify-content:center;font-family:'+SANS+';font-size:10px;font-weight:600;flex-shrink:0">EA</div>',
      '            <div style="flex:1;min-width:0"><div style="font-family:'+SANS+';font-size:12.5px;color:#222;line-height:1.4"><strong style="color:#0f3e17;font-weight:600">Elena Adeyemi</strong> onboarded Nova Health</div><div style="font-family:'+SANS+';font-size:10px;color:#666;margin-top:2px">1 h ago</div></div>',
      '            <span style="padding:5px 12px;border-radius:999px;background:#b1dbb8;color:#0f3e17;font-family:'+SANS+';font-size:10px;font-weight:600;white-space:nowrap">Onboarded</span>',
      '          </div>',
      '        </div>',
      '      </div>',
      '    </div>',
      ...
      // ─── CUSTOMERS VIEW ────────────────────────────────────────────
      '    <div class="poc-view" data-view="customers" style="display:none;padding:28px 32px;display:none;flex-direction:column;gap:20px">',
      '      <div style="display:flex;justify-content:space-between;align-items:flex-end">',
      '        <div>',
      '          <span style="display:inline-block;font-family:'+SANS+';font-size:11px;font-weight:600;letter-spacing:.09em;text-transform:uppercase;color:#0f3e17;margin-bottom:8px">CUSTOMERS</span>',
      '          <div style="font-family:'+SERIF+';font-size:30px;font-weight:300;line-height:1.1;color:#0f3e17;letter-spacing:-.02em">All customers</div>',
      '          <div style="font-family:'+SANS+';font-size:12px;color:#222;margin-top:4px">142 active &middot; 8 added this week</div>',
      '        </div>',
      '        <div class="poc-interactive" onclick="pocGoContact()" style="padding:10px 18px;background:#0f3e17;color:#fffefc;border-radius:14px;font-family:'+SANS+';font-size:12px;font-weight:400">+ Add Customer</div>',
      '      </div>',
      '      <div style="background:#fffefc;border:1px solid #efeeeb;border-radius:14px;overflow:hidden">',
      '        <div style="display:grid;grid-template-columns:2fr 1.5fr 1fr 1fr;padding:14px 20px;background:#e1f4df;font-family:'+SANS+';font-size:10px;font-weight:600;letter-spacing:.08em;text-transform:uppercase;color:#0f3e17">',
      '          <div>Customer</div><div>Plan</div><div>MRR</div><div>Status</div>',
      '        </div>',
      _pocCrmRow('JR', 'Jenna Robertson', 'jenna@acme.co', 'Growth',  '$149', 'Active',    '#e1f4df', SANS) +
      _pocCrmRow('TF', 'Trevor Ferraro', 'trevor@nova.io', 'Scale', '$499', 'Active',    '#e1f4df', SANS) +
      _pocCrmRow('EA', 'Elena Adeyemi',  'elena@arbor.co', 'Growth', '$149', 'Onboarding','#cfe7d3', SANS) +
      _pocCrmRow('OK', 'Omar Kelekci',   'omar@rivet.io',  'Starter','$49',  'Trial',     '#b6ced5', SANS) +
      _pocCrmRow('PL', 'Priya Larose',   'priya@lumen.co', 'Growth', '$149', 'Active',    '#e1f4df', SANS) +
      _pocCrmRow('DM', 'Daniel Morrow',  'daniel@quill.co','Scale',  '$499', 'Active',    '#e1f4df', SANS),
      '      </div>',
      '      <div style="text-align:center;font-family:'+SANS+';font-size:11px;color:#666;margin-top:8px">Showing 6 of 142 &middot; <span class="poc-interactive" onclick="pocGoContact()" style="color:#0f3e17;font-weight:500">Load more</span></div>',
      '    </div>',

      // ─── ANALYTICS VIEW ────────────────────────────────────────────
      '    <div class="poc-view" data-view="analytics" style="display:none;padding:28px 32px;flex-direction:column;gap:20px">',
      '      <div style="display:flex;justify-content:space-between;align-items:flex-end">',
      '        <div>',
      '          <span style="display:inline-block;font-family:'+SANS+';font-size:11px;font-weight:600;letter-spacing:.09em;text-transform:uppercase;color:#0f3e17;margin-bottom:8px">ANALYTICS</span>',
      '          <div style="font-family:'+SERIF+';font-size:30px;font-weight:300;line-height:1.1;color:#0f3e17;letter-spacing:-.02em">Revenue trend</div>',
      '          <div style="font-family:'+SANS+';font-size:12px;color:#222;margin-top:4px">Last 6 months</div>',
      '        </div>',
      '        <div class="poc-interactive" onclick="pocGoContact()" style="padding:10px 18px;background:#fffefc;color:#0f3e17;border:1px solid #efeeeb;border-radius:14px;font-family:'+SANS+';font-size:12px;font-weight:400">Export report &darr;</div>',
      '      </div>',
      // Chart card
      '      <div style="background:#e1f4df;border-radius:14px;padding:28px 24px 20px">',
      '        <svg viewBox="0 0 400 140" width="100%" height="140" preserveAspectRatio="none" style="display:block">',
      '          <defs><linearGradient id="pocAnalyticsGrad" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stop-color="#0f3e17" stop-opacity=".22"/><stop offset="100%" stop-color="#0f3e17" stop-opacity="0"/></linearGradient></defs>',
      '          <path d="M0,110 L60,88 L120,74 L180,58 L240,44 L300,32 L360,18 L400,10 L400,140 L0,140 Z" fill="url(#pocAnalyticsGrad)"/>',
      '          <path d="M0,110 L60,88 L120,74 L180,58 L240,44 L300,32 L360,18 L400,10" stroke="#0f3e17" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round"/>',
      '          <circle cx="400" cy="10" r="4" fill="#0f3e17"/>',
      '        </svg>',
      '        <div style="display:flex;justify-content:space-between;margin-top:10px;font-family:'+SANS+';font-size:10px;color:#666">',
      ...
      '      <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:14px">',
      '        <div style="background:#cfe7d3;border-radius:14px;padding:22px">',
      '          <span style="display:inline-block;font-family:'+SANS+';font-size:10px;font-weight:600;letter-spacing:.09em;text-transform:uppercase;color:#0f3e17;margin-bottom:10px">Growth Rate</span>',
      '          <div style="font-family:'+SERIF+';font-size:32px;font-weight:300;line-height:1;color:#0f3e17;letter-spacing:-.02em">+22%</div>',
      '          <div style="font-family:'+SANS+';font-size:11px;color:#222;margin-top:8px">QoQ average</div>',
      '        </div>',
      '        <div style="background:#b1dbb8;border-radius:14px;padding:22px">',
      '          <span style="display:inline-block;font-family:'+SANS+';font-size:10px;font-weight:600;letter-spacing:.09em;text-transform:uppercase;color:#0f3e17;margin-bottom:10px">Churn</span>',
      '          <div style="font-family:'+SERIF+';font-size:32px;font-weight:300;line-height:1;color:#0f3e17;letter-spacing:-.02em">1.8%</div>',
      '          <div style="font-family:'+SANS+';font-size:11px;color:#222;margin-top:8px">-0.4% MoM</div>',
      '        </div>',
      '        <div style="background:#b6ced5;border-radius:14px;padding:22px">',
      '          <span style="display:inline-block;font-family:'+SANS+';font-size:10px;font-weight:600;letter-spacing:.09em;text-transform:uppercase;color:#0f3e17;margin-bottom:10px">LTV / CAC</span>',
      '          <div style="font-family:'+SERIF+';font-size:32px;font-weight:300;line-height:1;color:#0f3e17;letter-spacing:-.02em">4.2x</div>',
      '          <div style="font-family:'+SANS+';font-size:11px;color:#222;margin-top:8px">Above target</div>',
      '        </div>',
      '      </div>',
      '    </div>',
  ...
  // Helper for a single Customers-table row in the CRM mockup.
  function _pocCrmRow(init, personName, email, plan, mrr, status, statusBg, sansFont) {
    return '<div class="poc-interactive" onclick="pocGoContact()" style="display:grid;grid-template-columns:2fr 1.5fr 1fr 1fr;padding:12px 20px;border-top:1px solid #efeeeb;align-items:center">' +
      '<div style="display:flex;align-items:center;gap:10px"><div style="width:26px;height:26px;border-radius:50%;background:#0f3e17;color:#fffefc;display:flex;align-items:center;justify-content:center;font-family:'+sansFont+';font-size:9px;font-weight:600;flex-shrink:0">'+init+'</div><div style="min-width:0"><div style="font-family:'+sansFont+';font-size:12px;font-weight:600;color:#0f3e17;line-height:1.1">'+personName+'</div><div style="font-family:'+sansFont+';font-size:10px;color:#666;margin-top:2px">'+email+'</div></div></div>' +
      '<div style="font-family:'+sansFont+';font-size:12px;color:#222">'+plan+'</div>' +
      '<div style="font-family:'+sansFont+';font-size:12px;color:#222">'+mrr+'</div>' +
      '<div><span style="padding:4px 10px;border-radius:999px;background:'+statusBg+';color:#0f3e17;font-family:'+sansFont+';font-size:10px;font-weight:600">'+status+'</span></div>' +
    '</div>';
  }
