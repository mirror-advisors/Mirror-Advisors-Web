...
var AI_SIGNUP_STEPS = [
  ...
  { key:'phone',       type:'tel',         prompt:function(a){ var n=(a.name||'').split(/\s+/)[0]; return (n?"Nice to meet you, "+n+". ":"")+"What's the best number to reach you on?"; }, placeholder:"+63 xxx xxx xxxx" },
  ...
  { key:'contactTime', type:'textarea',    prompt:"Last one — when and how should we reach you for the session?",
    placeholder:"e.g. Weekdays after 2pm PHT, email preferred" },
];
...
function aiSignupFinalize(){
  ...
  setTimeout(function(){
    ...
    // Fire-and-forget submission. If the endpoint doesn't exist yet the
    // user's confirmation still shows; we log the payload for now.
    try {
      console.log('[ai-signup] submitting:', window._aiSignup.answers);
      fetch('/api/ai-signup', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(window._aiSignup.answers),
      }).catch(function(){});
    } catch(_){}
  }, 900);
}
