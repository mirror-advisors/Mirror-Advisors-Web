...
import { saveConfig, getCurrentUser } from './site-config';
import { getSupabase } from './supabase';

// Fire-and-forget Supabase persistence helper used by admin Save handlers.
// Checks auth state first, logs clearly, surfaces failures to the admin via
// the existing toast helper. Never throws — UI stays responsive on any error.
function _persist(key, value) {
  (async function () {
    try {
      const user = await getCurrentUser();
      if (!user) {
        console.warn('[site-runtime] Not signed in — save to "' + key + '" skipped. Go to /admin/login.');
        if (window._adminToast) window._adminToast('⚠ Not signed in — go to /admin/login to save globally');
        return;
      }
      const ok = await saveConfig(key, value);
      if (ok) {
        console.log('[site-runtime] ✓ Persisted to Supabase: ' + key);
      } else {
        console.warn('[site-runtime] Save failed: ' + key + ' (check RLS policies + signed-in user)');
