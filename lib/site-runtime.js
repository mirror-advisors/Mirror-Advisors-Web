...
if (typeof window !== 'undefined') {
  ...
  window.pocGenerate = function () {
    ...
    // Render the preview immediately — instant client-side response
    // whether or not the notification email succeeds.
    var out = document.getElementById('pocPreview');
    if (out) {
      ...
      out.innerHTML =
        '<div class="poc-status" id="pocStatus"><span>Sending notification to Mirror Advisors&hellip;</span><span class="poc-status-dot"></span></div>' +
        mockup;
    }

    // Fire-and-forget POST to /api/poc-notify. Update the status ribbon
    // when it resolves either way.
    fetch('/api/poc-notify', {
      method:  'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        companyName:  name,
      ...
      .then(function (data) {
        var status = document.getElementById('pocStatus');
        if (!status) return;
        if (data && data.ok) {
          status.className = 'poc-status sent';
          status.innerHTML = '<span>Notification sent to Mirror Advisors &check;</span><span class="poc-status-dot"></span>';
        } else {
          status.className = 'poc-status failed';
          status.innerHTML = '<span>Preview generated. Notification failed &mdash; try /contact.</span><span class="poc-status-dot"></span>';
        }
      })
      .catch(function () {
        var status = document.getElementById('pocStatus');
        if (!status) return;
        status.className = 'poc-status failed';
        status.innerHTML = '<span>Preview generated. Network error &mdash; try /contact.</span><span class="poc-status-dot"></span>';
      });
  };
  ...
  function _pocRenderWebsite(name, details, logoDataUrl) {
    ...
    var logoBlock   = logoDataUrl
      ...
      : '<div style="width:28px;height:28px;border-radius:7px;background:linear-gradient(135deg,#ECA934,#B8881F);color:#0D1232;display:inline-flex;align-items:center;justify-content:center;font-family:Montserrat,sans-serif;font-size:12px;font-weight:900;letter-spacing:-.02em">' + initials + '</div>';

    return [
      '<div style="background:#0E1422;border:1px solid rgba(255,255,255,.08);border-radius:12px;overflow:hidden">',
      // Browser chrome
      ...
      '  <span style="margin-left:8px;font-size:10px;font-family:\'DM Sans\',sans-serif;color:rgba(255,255,255,.5);background:rgba(255,255,255,.04);padding:3px 10px;border-radius:6px">' + domain + '.com</span>',
      '  <span style="margin-left:auto;font-size:8px;padding:2px 7px;border-radius:100px;background:rgba(236,169,52,.18);color:#ECA934;font-weight:800;font-family:Montserrat,sans-serif;letter-spacing:.05em">POC PREVIEW</span>',
      '</div>',
      // Nav row
      '<div style="display:flex;align-items:center;justify-content:space-between;padding:14px 20px;border-bottom:1px solid rgba(255,255,255,.05)">',
      '  <div style="display:flex;align-items:center;gap:10px">',
      logoBlock,
      '    <div style="font-family:Montserrat,sans-serif;font-size:14px;font-weight:800;color:var(--tx);letter-spacing:-.01em">' + safeName + '</div>',
      '  </div>',
      '  <div style="display:flex;gap:14px;font-family:Montserrat,sans-serif;font-size:11px;font-weight:700;color:var(--dim)">',
      '    <span>Product</span><span>Pricing</span><span>About</span><span style="color:var(--t)">Contact</span>',
      '  </div>',
      '</div>',
      // Hero
      '<div style="padding:32px 24px 24px;position:relative;overflow:hidden">',
      '  <div style="position:absolute;top:-20px;right:-20px;width:200px;height:160px;border-radius:50%;background:radial-gradient(ellipse,rgba(236,169,52,.14),transparent 60%);pointer-events:none"></div>',
      '  <div style="position:relative">',
      '    <div style="font-size:9px;padding:3px 10px;border-radius:100px;background:rgba(236,169,52,.12);border:1px solid rgba(236,169,52,.28);color:var(--t);font-family:Montserrat,sans-serif;font-weight:800;letter-spacing:.09em;display:inline-block;margin-bottom:14px">NEW &middot; TRUSTED BY 100+ TEAMS</div>',
      '    <div style="font-family:Montserrat,sans-serif;font-size:24px;font-weight:800;color:var(--tx);line-height:1.1;margin-bottom:10px;letter-spacing:-.02em">Built for<br><span style="color:var(--t)">' + safeName + '</span> customers.</div>',
      '    <div style="font-size:12.5px;color:var(--dim);line-height:1.65;margin-bottom:18px;max-width:340px">' + safeDetails + '</div>',
      '    <div style="display:flex;gap:8px;margin-bottom:24px">',
      '      <div style="padding:10px 18px;background:var(--t);color:#0D1232;border-radius:8px;font-family:Montserrat,sans-serif;font-size:11px;font-weight:800">Get Started &rarr;</div>',
      '      <div style="padding:10px 18px;background:rgba(255,255,255,.06);color:var(--tx);border:1px solid rgba(255,255,255,.12);border-radius:8px;font-family:Montserrat,sans-serif;font-size:11px;font-weight:700">See how it works</div>',
      '    </div>',
      '  </div>',
      // 3 feature tiles under hero
      '  <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;position:relative">',
      '    <div style="padding:10px 12px;border-radius:8px;background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.08)"><div style="font-family:Montserrat,sans-serif;font-size:8px;font-weight:800;color:var(--t);letter-spacing:.08em;margin-bottom:3px">01</div><div style="font-family:Montserrat,sans-serif;font-size:10px;font-weight:800;color:var(--tx);margin-bottom:2px">Fast setup</div><div style="font-size:8.5px;color:var(--dim);line-height:1.4">Live in under a week</div></div>',
      '    <div style="padding:10px 12px;border-radius:8px;background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.08)"><div style="font-family:Montserrat,sans-serif;font-size:8px;font-weight:800;color:#6B9FD4;letter-spacing:.08em;margin-bottom:3px">02</div><div style="font-family:Montserrat,sans-serif;font-size:10px;font-weight:800;color:var(--tx);margin-bottom:2px">Zero lock-in</div><div style="font-size:8.5px;color:var(--dim);line-height:1.4">Full source-code handover</div></div>',
      '    <div style="padding:10px 12px;border-radius:8px;background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.08)"><div style="font-family:Montserrat,sans-serif;font-size:8px;font-weight:800;color:#8B9FD4;letter-spacing:.08em;margin-bottom:3px">03</div><div style="font-family:Montserrat,sans-serif;font-size:10px;font-weight:800;color:var(--tx);margin-bottom:2px">You own it</div><div style="font-size:8.5px;color:var(--dim);line-height:1.4">Your infrastructure, your team</div></div>',
      '  </div>',
      '</div>',
      // Footer strip
      '<div style="display:grid;grid-template-columns:1fr 1fr 1fr;border-top:1px solid rgba(255,255,255,.06);background:#080B16">',
      '  <div style="padding:12px 10px;text-align:center;border-right:1px solid rgba(255,255,255,.06)"><div style="font-family:Montserrat,sans-serif;font-size:9px;font-weight:800;color:var(--dim);letter-spacing:.08em;margin-bottom:2px">SPEED</div><div style="font-family:Montserrat,sans-serif;font-size:15px;font-weight:800;color:#10B981">98</div></div>',
      '  <div style="padding:12px 10px;text-align:center;border-right:1px solid rgba(255,255,255,.06)"><div style="font-family:Montserrat,sans-serif;font-size:9px;font-weight:800;color:var(--dim);letter-spacing:.08em;margin-bottom:2px">UPTIME</div><div style="font-family:Montserrat,sans-serif;font-size:15px;font-weight:800;color:var(--t)">99.9%</div></div>',
      '  <div style="padding:12px 10px;text-align:center"><div style="font-family:Montserrat,sans-serif;font-size:9px;font-weight:800;color:var(--dim);letter-spacing:.08em;margin-bottom:2px">CVR</div><div style="font-family:Montserrat,sans-serif;font-size:15px;font-weight:800;color:var(--t)">4.2%</div></div>',
      '</div>',
      '</div>',
    ].join('');
  }

  // A tiny mock CRM dashboard: sidebar with client's branding + main
  // pane with 3 stat cards + activity feed.
  function _pocRenderCrm(name, details, logoDataUrl) {
    ...
    var initials    = safeName.replace(/[^A-Za-z ]/g, '').split(/\s+/).map(function (w) { return w[0] || ''; }).join('').slice(0, 2).toUpperCase() || safeName.slice(0, 2).toUpperCase();
    var logoBlock   = logoDataUrl
      ? '<img src="' + logoDataUrl + '" alt="' + safeName + '" style="max-height:22px;max-width:60px;object-fit:contain;display:block;margin:0 auto">'
      : '<div style="font-family:Montserrat,sans-serif;font-weight:900;font-size:18px;color:#ECA934;letter-spacing:-.04em;line-height:1;text-align:center">' + initials + '</div>';
    ...
    return [
      ...
      '  <span style="width:9px;height:9px;border-radius:50%;background:#ffbd2e"></span>',
      '  <span style="width:9px;height:9px;border-radius:50%;background:#28c840"></span>',
      '  <span style="margin-left:8px;font-size:10px;font-family:\'DM Sans\',sans-serif;color:rgba(255,255,255,.5);background:rgba(255,255,255,.04);padding:3px 10px;border-radius:6px">app.' + domain + '.com/dashboard</span>',
      '  <span style="margin-left:auto;font-size:8px;padding:2px 7px;border-radius:100px;background:rgba(236,169,52,.18);color:#ECA934;font-weight:800;font-family:Montserrat,sans-serif;letter-spacing:.05em">POC PREVIEW</span>',
      '</div>',
      // Dashboard body: sidebar + main
      '<div style="display:grid;grid-template-columns:96px 1fr;min-height:400px">',
      // Sidebar
      '  <div style="background:#0E1422;padding:14px 0;display:flex;flex-direction:column;border-right:1px solid rgba(255,255,255,.05)">',
      '    <div style="padding:0 10px 12px;border-bottom:1px solid rgba(255,255,255,.05)">' + logoBlock + '<div style="font-family:Montserrat,sans-serif;font-size:6.5px;font-weight:800;color:rgba(255,255,255,.55);letter-spacing:.18em;margin-top:4px;text-align:center">' + safeName.slice(0, 14).toUpperCase() + '</div></div>',
      '    <div style="display:flex;flex-direction:column;gap:1px;padding:10px 6px;flex:1">',
      '      <div style="display:flex;align-items:center;gap:6px;padding:7px 8px;border-radius:6px;background:rgba(255,255,255,.06)"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="#fff" stroke-width="2"><rect x="3" y="3" width="7" height="9"/><rect x="14" y="3" width="7" height="5"/><rect x="14" y="12" width="7" height="9"/><rect x="3" y="16" width="7" height="5"/></svg><span style="font-size:8px;color:#fff;font-family:\'DM Sans\',sans-serif;font-weight:700">Dashboard</span></div>',
      '      <div style="display:flex;align-items:center;gap:6px;padding:7px 8px;color:rgba(255,255,255,.55)"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/></svg><span style="font-size:8px;font-family:\'DM Sans\',sans-serif">Customers</span></div>',
      '      <div style="display:flex;align-items:center;gap:6px;padding:7px 8px;color:rgba(255,255,255,.55)"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="18" y1="20" x2="18" y2="10"/><line x1="12" y1="20" x2="12" y2="4"/><line x1="6" y1="20" x2="6" y2="14"/></svg><span style="font-size:8px;font-family:\'DM Sans\',sans-serif">Analytics</span></div>',
      '      <div style="display:flex;align-items:center;gap:6px;padding:7px 8px;color:rgba(255,255,255,.55)"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg><span style="font-size:8px;font-family:\'DM Sans\',sans-serif">Docs</span></div>',
      '      <div style="display:flex;align-items:center;gap:6px;padding:7px 8px;color:rgba(255,255,255,.55)"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="3"/><path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 0 1 0 2.83 2 2 0 0 1-2.83 0l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 0 1-4 0v-.09a1.65 1.65 0 0 0-1-1.51 1.65 1.65 0 0 0-1.82.33l-.06.06a2 2 0 0 1-2.83 0 2 2 0 0 1 0-2.83l.06-.06a1.65 1.65 0 0 0 .33-1.82 1.65 1.65 0 0 0-1.51-1H3a2 2 0 0 1 0-4h.09a1.65 1.65 0 0 0 1.51-1 1.65 1.65 0 0 0-.33-1.82l-.06-.06a2 2 0 0 1 0-2.83 2 2 0 0 1 2.83 0l.06.06a1.65 1.65 0 0 0 1.82.33H9a1.65 1.65 0 0 0 1-1.51V3a2 2 0 0 1 4 0v.09a1.65 1.65 0 0 0 1 1.51 1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 0 1 2.83 0 2 2 0 0 1 0 2.83l-.06.06a1.65 1.65 0 0 0-.33 1.82V9a1.65 1.65 0 0 0 1.51 1H21a2 2 0 0 1 0 4h-.09a1.65 1.65 0 0 0-1.51 1z"/></svg><span style="font-size:8px;font-family:\'DM Sans\',sans-serif">Settings</span></div>',
      '    </div>',
      '    <div style="display:flex;align-items:center;gap:6px;padding:9px 10px;border-top:1px solid rgba(255,255,255,.05)"><div style="width:16px;height:16px;border-radius:50%;background:#ECA934;color:#0F1424;display:flex;align-items:center;justify-content:center;font-size:7px;font-weight:800;font-family:Montserrat,sans-serif;flex-shrink:0">AK</div><div style="min-width:0"><div style="font-size:7px;color:#fff;font-weight:700;font-family:Montserrat,sans-serif;line-height:1.1">Alex Kim</div><div style="font-size:6px;color:rgba(255,255,255,.45)">Admin</div></div></div>',
      '  </div>',
      // Main content
      '  <div style="background:#F7F8FA;padding:16px 18px;color:#1A2238;font-family:\'DM Sans\',sans-serif">',
      '    <div style="display:flex;justify-content:space-between;align-items:center;padding-bottom:10px;border-bottom:1px solid #E8EBF0;margin-bottom:14px"><div style="font-family:Montserrat,sans-serif;font-size:12px;font-weight:800;color:#1A2238">Dashboard</div><div style="display:flex;gap:7px;align-items:center"><div style="width:20px;height:20px;border-radius:50%;background:#E8EBF0;display:flex;align-items:center;justify-content:center;position:relative;color:#5B6478;font-size:9px">&#128276;<span style="position:absolute;top:2px;right:3px;width:5px;height:5px;border-radius:50%;background:#EF4444"></span></div><div style="width:20px;height:20px;border-radius:50%;background:#0E1422;color:#fff;display:flex;align-items:center;justify-content:center;font-size:7px;font-weight:800;font-family:Montserrat,sans-serif">AK</div></div></div>',
      '    <div style="font-family:Montserrat,sans-serif;font-size:17px;font-weight:800;color:#0F1424;margin-bottom:2px;line-height:1.1;letter-spacing:-.01em">Good afternoon, Alex</div>',
      '    <div style="font-size:9px;color:#6B7488;margin-bottom:14px">' + safeName + ' &middot; ' + safeDetails + '</div>',
      // 3 stat cards
      '    <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;margin-bottom:14px">',
      '      <div style="padding:11px 12px;border-radius:8px;background:#fff;border:1px solid #E8EBF0"><div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:5px"><div style="font-size:8px;color:#6B7488;font-weight:700">Active Customers</div><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="#6B7488" stroke-width="2"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/></svg></div><div style="font-family:Montserrat,sans-serif;font-size:20px;font-weight:800;color:#0F1424;line-height:1">142</div><div style="font-size:7px;color:#10B981;margin-top:4px;font-weight:700">+8 this week</div></div>',
      '      <div style="padding:11px 12px;border-radius:8px;background:#fff;border:1px solid #E8EBF0"><div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:5px"><div style="font-size:8px;color:#6B7488;font-weight:700">Open Tickets</div><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="#6B7488" stroke-width="2"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg></div><div style="font-family:Montserrat,sans-serif;font-size:20px;font-weight:800;color:#0F1424;line-height:1">23</div><div style="font-size:7px;color:#EF4444;margin-top:4px;font-weight:700">2 SLA at risk</div></div>',
      '      <div style="padding:11px 12px;border-radius:8px;background:#fff;border:1px solid #E8EBF0"><div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:5px"><div style="font-size:8px;color:#6B7488;font-weight:700">MRR</div><span style="font-size:9px;color:#6B7488">$</span></div><div style="font-family:Montserrat,sans-serif;font-size:16px;font-weight:800;color:#0F1424;line-height:1">$142,400</div><div style="font-size:7px;color:#10B981;margin-top:4px;font-weight:700">+22% QoQ</div></div>',
      '    </div>',
      // Activity feed
      '    <div style="padding:11px 13px;border-radius:8px;background:#fff;border:1px solid #E8EBF0">',
      '      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:9px"><span style="font-family:Montserrat,sans-serif;font-size:10px;font-weight:800;color:#0F1424">Recent Activity</span><span style="font-size:7px;color:#A78BFA;font-weight:700;font-family:Montserrat,sans-serif">View All</span></div>',
      '      <div style="display:flex;flex-direction:column;gap:7px">',
      '        <div style="display:flex;gap:6px;align-items:flex-start"><div style="width:15px;height:15px;border-radius:50%;background:#0E1422;color:#fff;display:flex;align-items:center;justify-content:center;font-size:6px;font-weight:800;font-family:Montserrat,sans-serif;flex-shrink:0">JR</div><div style="flex:1;min-width:0"><div style="font-size:8px;color:#1A2238;line-height:1.35"><strong>Jenna Robertson</strong> upgraded to Growth <span style="color:#A78BFA">Acme Co.</span></div><div style="font-size:6px;color:#9AA3B5;margin-top:2px">Just now</div></div></div>',
      '        <div style="display:flex;gap:6px;align-items:flex-start"><div style="width:15px;height:15px;border-radius:50%;background:#0E1422;color:#fff;display:flex;align-items:center;justify-content:center;font-size:6px;font-weight:800;font-family:Montserrat,sans-serif;flex-shrink:0">TF</div><div style="flex:1;min-width:0"><div style="font-size:8px;color:#1A2238;line-height:1.35"><strong>Trevor Ferraro</strong> resolved 3 tickets <span style="color:#A78BFA">Support queue</span></div><div style="font-size:6px;color:#9AA3B5;margin-top:2px">18 min ago</div></div></div>',
      '        <div style="display:flex;gap:6px;align-items:flex-start"><div style="width:15px;height:15px;border-radius:50%;background:#0E1422;color:#fff;display:flex;align-items:center;justify-content:center;font-size:6px;font-weight:800;font-family:Montserrat,sans-serif;flex-shrink:0">EA</div><div style="flex:1;min-width:0"><div style="font-size:8px;color:#1A2238;line-height:1.35"><strong>Elena Adeyemi</strong> onboarded a new team <span style="color:#A78BFA">Nova Health</span></div><div style="font-size:6px;color:#9AA3B5;margin-top:2px">1 h ago</div></div></div>',
      '      </div>',
      '    </div>',
      '  </div>',
      '</div>',
