...
function adminRenderCasesDash(container){
  // NOTE: no Supabase fetch here. window._CASES is already populated on page
  // load — first by initSiteRuntime's hardcoded defaults, then overwritten
  // by Layout.js's fetchAllConfig() if the user has saved before. This
  // function is a PURE render — it never re-fetches, never re-renders itself
  // asynchronously, never wipes the editor mid-edit. Saves still write
  // through to Supabase via _persist('cases', ...) in adminSaveCase.
...
window.adminEditCase = function(idx){
  console.log('[admin] adminEditCase called for idx:', idx, 'case:', window._CASES[idx]);
  try {
    _adminIdx = idx;
    ...
    var c = window._CASES[idx];
    if(!c){
      console.warn('[admin] No case at index ' + idx + ' (length: ' + window._CASES.length + ')');
      return;
    }
    adminRenderEditor(c, false);
    console.log('[admin] Editor should be visible now. adminEditorBody innerHTML length:',
      (document.getElementById('adminEditorBody') || {}).innerHTML ? document.getElementById('adminEditorBody').innerHTML.length : 0);
  } catch(err){
    console.error('[admin] adminEditCase threw:', err);
    var eb = document.getElementById('adminEditorBody');
    if(eb) eb.innerHTML = '<div style="padding:24px;color:#FCA5A5;font-family:monospace;font-size:12px;line-height:1.6">' +
      '<strong>Editor error:</strong><br>' + (err && err.message ? err.message : String(err)) +
      '<br><br>Check the browser console for the full stack trace.</div>';
  }
};
